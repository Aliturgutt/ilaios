...
_PATTERNS = (
    ...
    _Pattern(
        ...
        "inline", "content-sized", "wrap-without-horizontal-overflow",
        ("open-list", "toggle-option", "clear-selection"),
        ("labelled-control", "keyboard-navigation", "announced-selection-state", "visible-focus"),
    ),
    _Pattern(
        "avatar-group", "display",
        ("avatar group", "stacked avatars", "overlapping avatars", "ust uste avatar", "ust uste kucuk kullanici resimleri"),
        "inline", "content-sized", "cap-visible-avatars-and-show-overflow-count",
        ("show-overflow-count",),
    ...
    _Pattern(
        ...
        "inline", "container-bound", "preserve-full-value-discoverability",
    ...
    _Pattern(
        ...
        "center", "min(560px,calc(100vw-32px))", "safe-near-full-width",
        ("escape-close", "explicit-actions", "restore-trigger-focus"),
        ("focus-trap", "dialog-semantics", "labelled-title", "keyboard-operable", "visible-focus"),
    ),
    _Pattern(
        "tabs", "navigation",
        ("tabs", "tab bar", "sekmeler", "sekme", "tab navigation"),
        "inline", "container-width", "intentional-compact-navigation",
        ("activate-tab", "preserve-selected-state"),
        ("tablist-semantics", "arrow-key-navigation", "visible-focus"),
    ),
    _Pattern(
        "command-palette", "navigation",
        ("command palette", "command menu", "komut paleti", "hizli komut", "quick command"),
        "center", "640px", "bounded-full-width-overlay",
    ...
    _Pattern(
        ...
        "inline", "container-width", "preserve-critical-columns",
    ...
    _Pattern(
        ...
        "bottom-right", "360px", "respect-safe-areas",
    ...
    _Pattern(
        ...
        "page", "responsive-container", "intentional-compact-tablet-wide-composition",
...
_DIAGRAM_SIGNALS = (
    "diagram", "flowchart", "flow chart", "architecture diagram", "mimari diyagram",
    "node graph", "dag", "sequence diagram", "class diagram", "system map",
)
...
def resolve_ui_design(prompt: str, *, product: str | None = None) -> UIDesignSpec:
    ...
    diagram_hits = tuple(signal for signal in _DIAGRAM_SIGNALS if _contains(normalized, signal))
    ...
    for pattern in _PATTERNS:
        ...
        if hits:
            ...
            score = min(0.98, 0.68 + 0.07 * min(longest, 4) + 0.03 * (len(hits) - 1))
    ...
    if diagram_hits:
        raise UIDesignError("ambiguous UI/diagram intent must be resolved before execution")
    if len(candidates) > 1 and candidates[0][0] - candidates[1][0] <= 0.05:
        raise UIDesignError("ambiguous UI component intent must be resolved before execution")

    score, pattern, hits = candidates[0]
    placement = _placement(normalized, pattern.placement)
    brand_policy = "ILAIOS-canonical-tokens" if product and product.casefold() == "ilaios" else "inherit-target-product-brand"
    return UIDesignSpec(
...
def _normalize(prompt: str) -> str:
    ...
    without_marks = "".join(char for char in decomposed if unicodedata.category(char) != "Mn").replace("ı", "i")
