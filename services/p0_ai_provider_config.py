"""Strict external configuration loader for P0 governed AI providers.

No model, provider price, credential, tenant budget, or fallback is guessed from
an environment suffix or marketing label. The product user never chooses a
model; operators/runtime configuration provide an eligible pool and the Core
routing policy selects within it.
"""
...
def load_p0_ai_provider_configuration(
    ...
    """Load optional configuration from ``ILAIOS_AGENT_AI_CONFIG_JSON``.

    Absence is not an error: provider-backed P0 agents remain unavailable and
    therefore cannot be promoted. Presence must be complete and valid or the
    sidecar fails closed during startup.
    """
