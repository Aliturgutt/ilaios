...
class MicrosoftOIDCEnvironment:
    ...
    def from_environment(cls, env: Mapping[str, str]) -> "MicrosoftOIDCEnvironment":
        ...
        return cls(production_web_client_id, development_web_client_id, desktop_client_id, redirects)
...
def verified_microsoft_identity(claims: VerifiedOIDCClaims) -> VerifiedExternalIdentity:
...
def _validated_token_issuer(value: str) -> str:
    ...
    if not tenant_id or "/" in tenant_id or tenant_id in {"common", "organizations", "consumers"}:
        raise CentralIdentityError("Microsoft token issuer must contain a concrete tenant")
    return issuer
