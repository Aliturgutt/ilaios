...
class CommercialPlan:
    ...
    def __post_init__(self) -> None:
        if self.max_concurrent_jobs is not None and self.max_concurrent_jobs < 1:
            raise CommercialPlanError("max_concurrent_jobs must be positive when configured")
        if self.plan_id is not CommercialPlanId.ENTERPRISE and self.max_concurrent_jobs is None:
            raise CommercialPlanError("non-enterprise plans require max_concurrent_jobs")
        for name in ("workspace_users","max_active_projects","max_active_automations","automation_runs_per_month","storage_limit_gb","history_evidence_retention_days","monthly_video_pool_mini_480p_equivalent_minutes"):
        ...
        if self.plan_id is CommercialPlanId.FREE:
            ...
            if self.max_video_resolution is not None:
                raise CommercialPlanError("FREE cannot promise a fixed paid-video resolution")
            ...
            if self.max_video_resolution is None:
                raise CommercialPlanError("paid plans require a locked maximum video resolution")
            ...
            if self.max_concurrent_jobs is not None:
                raise CommercialPlanError("ENTERPRISE concurrent jobs must remain contract-specific")
            ...
            if not self.enterprise_custom_video_budget:
                raise CommercialPlanError("ENTERPRISE video allowance must remain contract-specific")
...
CommercialPlanId.ENTERPRISE: CommercialPlan(CommercialPlanId.ENTERPRISE,CommercialPlanId.POWER,None,None,None,None,None,None,None,None,None,True,None,("Seedance 2.0 Mini","Seedance 2.0 Fast","Seedance 2.0","contract-allowlisted-models"),"4K",None,(),False,True),
}
