...
Executing a verifier is not itself proof of correctness. This module accepts
only a strict verifier verdict that names the exact producer evidence digest.
Producer and verifier identities must be distinct and must match the canonical
...
def compile_independent_verification(
    ...
    """Compile a strict verifier result bound to one producer execution."""
...
def verification_prompt_payload(producer: ProviderBackedAgentResult) -> dict[str, object]:
    """Return the canonical evidence envelope supplied to an independent verifier."""
