...
def _http_json(path: str) -> tuple[int, dict[str, object]]:
    ...
    context = ssl._create_unverified_context()  # canary ALB DNS is not certificate name
    try:
...
def _backup_restore_drill(root: Path) -> dict[str, object]:
    ...
    if not isinstance(subnets, list) or not subnets or not isinstance(groups, list) or not groups:
    ...
    if isinstance(failures, list) and failures:
        raise OperationalEvidenceError(f"ECS maintenance task failed to start: {failures}")
    tasks = response.get("tasks")
    if not isinstance(tasks, list) or len(tasks) != 1 or not isinstance(tasks[0], dict):
        raise OperationalEvidenceError("ECS maintenance task was not created exactly once")
    ...
    if exit_code != 0:
        raise OperationalEvidenceError(
            f"backup/restore maintenance task exited with code {exit_code}: {task.get('stoppedReason')}"
        )
                ...
                if isinstance(events, list):
                    for event in events:
                        if not isinstance(event, dict):
                            continue
                        try:
                            payload: object = json.loads(str(event.get("message", "")))
                        except json.JSONDecodeError:
                            continue
                        if isinstance(payload, dict) and payload.get("event") == "rag14_backup_restore":
                            report = cast(dict[str, object], payload)
        if report is not None:
            break
        time.sleep(3)
    if report is None:
        raise OperationalEvidenceError("backup/restore report was not found in task logs")
    if report.get("status") != "PASS" or report.get("corrupt_restore_rejected") is not True:
        raise OperationalEvidenceError("backup/restore evidence did not PASS fail-closed checks")
    if report.get("production_authority") is not False:
        raise OperationalEvidenceError("backup drill attempted to claim production authority")
...
def _health_window(root: Path) -> dict[str, object]:
    ...
    for index in range(12):
        ...
        if status != 200 or payload.get("status") != "ready":
            raise OperationalEvidenceError("canary failed the sustained deployment health window")
        if index != 11:
            time.sleep(10)
...
def _metric_payload(metric_name: str, value: float, source_sha: str) -> dict[str, object]:
    return {
...
def _wait_alarm_state(names: list[str], expected: str, timeout: float = 90.0) -> dict[str, str]:
    deadline = time.monotonic() + timeout
    latest: dict[str, str] = {}
    while time.monotonic() < deadline:
        latest = _alarm_states(names)
        if len(latest) == len(names) and all(latest.get(name) == expected for name in names):
            return latest
        time.sleep(5)
    raise OperationalEvidenceError(
...
def _observability_alarm_drill(root: Path) -> dict[str, object]:
    ...
    try:
        for (label, metric_name), alarm_name in zip(rules.items(), alarm_names, strict=True):
            _run(
                "aws",
                "cloudwatch",
                "put-metric-alarm",
        ...
        _put_metrics(
            [_metric_payload(metric_name, 1.0, source_sha) for metric_name in rules.values()]
        )
        alarm_state = _wait_alarm_state(alarm_names, "ALARM")
        _put_metrics(
            [_metric_payload(metric_name, 0.0, source_sha) for metric_name in rules.values()]
        )
...
def _finops_meter(root: Path) -> dict[str, object]:
    ...
    efs_bytes = int(systems[0].get("SizeInBytes", {}).get("Value", 0)) if systems else 0
    report: dict[str, object] = {
        ...
        "currency_cost_note": "AWS billing currency cost is not claimed before provider billing evidence settles.",
        "production_authority": False,
    }
...
def _previous_release_is_verified(task_definition: str) -> dict[str, object]:
    ...
    if not source_tags:
        raise OperationalEvidenceError("previous task image lacks exact r01 source-SHA tag")
    source_sha = source_tags[0][4:]
    conclusion = _run(
        "gh",
        "api",
        f"/repos/{_required_env('GITHUB_REPOSITORY')}/actions/runs?head_sha={source_sha}&event=push&status=completed&per_page=100",
        "--jq",
        '[.workflow_runs[] | select(.name == "Required CI Gate" and .conclusion == "success")][0].conclusion // empty',
    )
    if conclusion != "success":
        raise OperationalEvidenceError("previous task artifact lacks exact-SHA Required CI PASS")
    return {"task_definition": task_definition, "image": image, "source_sha": source_sha}
...
def _rollback_drill(root: Path) -> dict[str, object]:
    ...
    if previous == current:
        raise OperationalEvidenceError("rollback requires a distinct previous task definition")
    ...
    try:
        ...
        if rollback_status != 200 or rollback_health.get("status") != "ready":
            raise OperationalEvidenceError("previous verified artifact did not recover health")
    finally:
        _run(
            "aws",
            "ecs",
    ...
    if status != 200:
        raise OperationalEvidenceError("current RAG.14 runtime did not recover after rollback drill")
    if verification.get("event_chain") != "verified" or verification.get("vector_index_integrity") is not True:
        raise OperationalEvidenceError("Knowledge integrity failed after rollback/roll-forward")
    report: dict[str, object] = {
        "status": "PASS",
