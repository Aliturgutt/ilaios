...
def evaluate_generated_web_product(
    files: Mapping[str, str],
    *,
    content_security_policy: str | None,
) -> GeneratedSecurityResult:
    """Fail-closed static acceptance gate for untrusted generated/revised Web products.

    This evaluator is validation infrastructure only. It grants no runtime, deployment,
    credential, policy, approval, Tool Gateway, tenant, or publication authority.
    """
    if not files:
    ...
    for path, source in sorted(files.items()):
        ...
        if "preinstall" in lowered or "postinstall" in lowered or "prepare" in lowered:
            if normalized_path.endswith("package.json"):
                findings.append(
                    GeneratedSecurityFinding(
                        "malicious-dependency",
                        normalized_path,
                        "package-install-hook-present",
                    )
                )

    findings.extend(_validate_csp(content_security_policy))
    return GeneratedSecurityResult(
