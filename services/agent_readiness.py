...
# These are implementation-boundary names already used by the canonical
# registry. They are not parallel capability identities or orchestration keys.
...
def p0_registrations() -> tuple[AgentRegistration, ...]:
    """Return the exact Core + Engineering + Security P0 population."""
    return tuple(
...
def audit_agent_registry(
    ...
    """Fail closed on duplicate, legacy, dead, or structurally unsafe identities."""
    if len(registrations) != EXPECTED_AGENT_COUNT:
        raise AgentReadinessError(
            f"canonical agent count mismatch: expected={EXPECTED_AGENT_COUNT} "
            f"actual={len(registrations)}"
        )
    ...
    if team_counts != EXPECTED_TEAM_COUNTS:
        raise AgentReadinessError(
            f"canonical team composition mismatch: {team_counts!r}"
        )
    ...
    for item in registrations:
        ...
        if item.backing_capability not in KNOWN_BACKING_CAPABILITIES:
            raise AgentReadinessError(
                f"unknown backing capability for {manifest.agent_id}: "
                f"{item.backing_capability}"
            )
        if manifest.dependencies != frozenset({item.backing_capability}):
            raise AgentReadinessError(
                f"backing capability/dependency mismatch for {manifest.agent_id}"
            )
        # Static registry promotion is prohibited. Runtime maturity is derived
        # only from AgentReadinessProof below.
...
def effective_readiness(proof: AgentReadinessProof) -> RuntimeReadiness:
    """Derive readiness strictly from the required execution/evidence sequence."""
