...
def compile_web_app_interaction_contract(
    ...
    """Compile the required lifecycle contracts without implementing Phase-5 runtime."""
...
def _canonical_machines(spec: WebAppSpec) -> tuple[WebAppStateMachineContract, ...]:
    ...
    return (
        WebAppStateMachineContract(
            ...
            transitions=(
                _transition("workflow-plan-execute", "Planning", "Executing", "workflow.execute", "project.manage", workflow_projection),
                _transition("workflow-execute-validate", "Executing", "Validation", "workflow.validate", "project.manage", workflow_projection),
                _transition("workflow-validate-deliver", "Validation", "Delivery", "workflow.deliver", "project.manage", workflow_projection),
                _transition("workflow-deliver-complete", "Delivery", "Completed", "workflow.complete", "project.manage", workflow_projection),
            ),
        ),
        WebAppStateMachineContract(
            machine_id="Approval",
            domain_subject="approval",
            ...
            transitions=(
                _transition("approval-pending-reviewing", "Pending", "Reviewing", "approval.review", "approval.review", "notification"),
                _transition("approval-reviewing-approved", "Reviewing", "Approved", "approval.approve", "approval.review", "notification"),
                _transition("approval-reviewing-rejected", "Reviewing", "Rejected", "approval.reject", "approval.review", "notification"),
            ),
        ),
        WebAppStateMachineContract(
            machine_id="Evidence",
            domain_subject="evidence",
            ...
            transitions=(
                _transition("evidence-generated-reviewed", "Generated", "Reviewed", "evidence.review", "evidence.review", "notification"),
                _transition("evidence-reviewed-verified", "Reviewed", "Verified", "evidence.verify", "evidence.review", "notification"),
                _transition("evidence-reviewed-failed", "Reviewed", "Failed", "evidence.fail", "evidence.review", "notification"),
            ),
        ),
    )
...
def _transition(
    ...
    return WebAppTransitionContract(
        ...
        to_state=to_state,
        ui_event=f"ui:{action_name}",
        api_mutation_id=f"mutation:{action_name}",
        auth_action_id=f"action:{permission}",
