...
class _RefreshCredentialStore:
    ...
    def load(self) -> _StoredRefreshCredential | None:
        ...
        try:
            ...
            token = self._unprotect(base64.b64decode(protected, validate=True)).decode("utf-8")
            if not token:
                raise ValueError("persistent identity credential is empty")
            return _StoredRefreshCredential(provider_id.strip(), token)
        except Exception as error:
            self.clear()
            raise DesktopIdentityError("Desktop persistent identity could not be restored") from error
    ...
    def save(self, provider_id: str, refresh_token: str) -> None:
        ...
        if not normalized_provider or not normalized_token:
            raise DesktopIdentityError("Desktop persistent identity credential is invalid")
        self._path.parent.mkdir(parents=True, exist_ok=True)
        protected = base64.b64encode(self._protect(normalized_token.encode("utf-8"))).decode("ascii")
        document = json.dumps(
            {
                "version": 1,
...
class _RefreshOIDCTokenVerifier:
    ...
    def verify(self, encoded_token: str) -> VerifiedOIDCClaims:
        try:
            ...
            key = jwt.PyJWKClient(self._provider.jwks_uri).get_signing_key_from_jwt(encoded_token)
            claims = jwt.decode(
                encoded_token,
                key.key,
        ...
        authentication_methods = (
            frozenset(value for value in methods if isinstance(value, str) and value.strip())
...
class DesktopOIDCService(_BaseDesktopOIDCService):
    ...
    def __init__(
        ...
        *,
        ...
        credential_store: _RefreshCredentialStore | None = None,
    ) -> None:
        ...
        self._credential_store = credential_store or _default_credential_store()
    ...
    def status(
        ...
        if state == _RESTORE_STATE:
            ...
            if restored is not None:
                return restored
            provider_id = next(iter(sorted(self._providers)), "google")
...
def _default_credential_store() -> _RefreshCredentialStore | None:
    if os.name != "nt" or not os.environ.get("ILAIOS_CONTROL_PLANE_TOKEN", "").strip():
        return None
