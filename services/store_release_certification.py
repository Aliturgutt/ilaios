...
# build receipt and device receipt validation across Phases 5-10.
# Golden references are exact-SHA256 locked and fail-closed per the truth rule.
# NOTE: In production, these would be content-addressed to actual certified binary
# identities. Currently locked to the deterministic placeholder to enable
# contract testing and framework development without external dependencies.
_ANDROID_GOLDEN_REFERENCE_SHA256 = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"


def validate_android_golden_reference(artifact_sha256: str, expected_sha256: str = _ANDROID_GOLDEN_REFERENCE_SHA256) -> None:
    """Validate Android artifact against golden reference SHA256.

    Fail-closed: raises StoreCertificationError if SHA256 does not match
    the canonical golden reference for the artifact type.

    This is a Phase 4 contract: downstream phases (5-10) should replace this
    placeholder with content-addressed golden references keyed to actual certified
    binary identities per ADR-0004 Runtime Role Separation and ADR-0005 Bounded
    Factories Shared Runtime.

    Args:
        artifact_sha256: The SHA256 of the actual artifact bytes
        expected_sha256: The canonical golden reference SHA256 (Phase 4 placeholder)

    Raises:
        StoreCertificationError: If artifact_sha256 != expected_sha256
    """
    if artifact_sha256 != expected_sha256:
        raise StoreCertificationError(
            f"artifact SHA256 {artifact_sha256[:16]}... does not match "
            f"golden reference {expected_sha256[:16]}...; "
            f"golden reference validation failed - see ADR-0004 and ADR-0005"
        )
