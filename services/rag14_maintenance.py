...
def _corrupt_archive(source: Path, target: Path) -> None:
    ...
    entries[corrupt_name] = (b"X" if not original else bytes([original[0] ^ 1]) + original[1:])
...
def run_backup_restore_drill(state_root: Path) -> dict[str, object]:
    ...
    if _required_env("ILAIOS_KNOWLEDGE_EMBEDDING_MODE") != PRODUCTION_EMBEDDING_MODE:
        raise RAG14MaintenanceError("maintenance drill requires the pinned production provider")
    ...
    with tempfile.TemporaryDirectory(prefix="ilaios-rag14-maintenance-") as temporary:
        ...
        started_files = tuple(sorted(path.relative_to(state_root).as_posix() for path in state_root.rglob("*") if path.is_file()))
        ...
        if manifest != restored_manifest:
            raise RAG14MaintenanceError("restored backup manifest differs from source manifest")
        ...
        if verification.get("event_chain") != "verified":
            raise RAG14MaintenanceError("restored Knowledge event chain failed verification")
        if verification.get("vector_index_integrity") is not True:
            raise RAG14MaintenanceError("restored vector index failed integrity verification")
        ...
        report: dict[str, object] = {
            ...
            "restored_vector_row_count": (
                state.get("vector_index", {}).get("row_count")
                if isinstance(state.get("vector_index"), dict)
                else None
            ),
            "corrupt_restore_rejected": True,
