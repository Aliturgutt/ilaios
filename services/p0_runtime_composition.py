...
This module never constructs a GovernedRuntime. It receives the already
canonical runtime and grant policy from the application composition root,
provisions exact canonical identities/immutable skills/providers idempotently,
and returns bounded executors over that same runtime.
"""
...
def compose_p0_runtime(
    ...
    *,
    ...
    """Provision P0 onto one existing runtime without minting parallel authority.

    P0 contains 21 target agents. ``IndependentVerifier`` is additionally
    provisioned as a verification dependency because Core/Engineering and the
    SecurityVerifier ultimately depend on it; it does not change the P0 target
    count.
    """
    p0 = p0_registrations()
    if len(p0) != 21:
    ...
    if ai_adapter is None:
        ...
        governed_ai_capabilities = {
            binding.capability
            for binding in __import__(
                "services.p0_agent_execution", fromlist=["P0_AGENT_BINDINGS"]
            ).P0_AGENT_BINDINGS
        ...
        for provider_id, provider_capabilities in sorted(capabilities.items()):
            ...
            if not provider_capabilities.issubset(governed_ai_capabilities | {"evidence.verify"}):
                raise P0RuntimeCompositionError(
