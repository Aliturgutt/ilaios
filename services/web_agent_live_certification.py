...
class BrowserToolEvidenceRuntimeAdapter:
    ...
    def execute(self, payload: dict[str, Any]) -> dict[str, object]:
        if payload.get("summary_path") != str(self._summary):
            raise WebAgentLiveCertificationError("BrowserQA evidence path drifted")
        if payload.get("source_sha") != self._source_sha:
...
def run_web_agent_live_certification(
    *, repository_root: Path, browser_summary_path: Path, output_dir: Path, now: datetime | None = None
) -> dict[str, object]:
    ...
    for binding in WEB_AGENT_BINDINGS:
        ...
        invocation = _invocation(
            binding.agent_id, binding.capability, binding.permission, prompt=prompt, external_egress=True
        )
        ...
        if not verification.passed:
            raise WebAgentLiveCertificationError(f"IndependentVerifier rejected {binding.agent_id}")
        ...
        receipts.append(_receipt(binding.agent_id, proof, producer.execution.route, verification.verifier_evidence_digest, verification.provider_id, verification.model_id))

    browser_binding = next(binding for binding in WEB_AGENT_BINDINGS if binding.execution_mode == "browser-tool")
    browser_prompt = "Bind the exact real Docker-isolated Playwright CLI evidence for this trusted source revision. Do not claim public-production verification."
    browser_execution = p0.named_executor.execute(
        _invocation(browser_binding.agent_id, browser_binding.capability, browser_binding.permission, prompt=browser_prompt, external_egress=False),
    ...
    receipts.append(_receipt(browser_binding.agent_id, browser_proof, browser_execution.route, browser_verification.verifier_evidence_digest, browser_verification.provider_id, browser_verification.model_id))

    if len(proofs) != 6:
        raise WebAgentLiveCertificationError("Web proof coverage must be 6/6")
    persisted = readiness_store.verify()
    if len(persisted) != 6 or any(record.readiness is not RuntimeReadiness.VERIFIED for record in persisted):
    ...
    (output_dir / "web-agent-live-receipt.json").write_text(json.dumps(receipt, sort_keys=True, indent=2) + "\n", encoding="utf-8")
    return receipt


def _invocation(agent_id: str, capability: str, permission: str, *, prompt: str, external_egress: bool) -> AgentInvocation:
...
def _persist_verified_proof(*, runtime: GovernedRuntime, store: AgentReadinessStore, agent_id: str, evidence_digest: str, created_at: datetime) -> AgentReadinessProof:
    ...
    _assert_projected_verified(agent_state_projection(runtime.routes(), store.projection()), {agent_id})
    return proof
...
def _assert_projected_verified(projection: dict[str, object], expected: set[str]) -> None:
    ...
    indexed = {str(item.get("agent_id")): item for item in agents if isinstance(item, dict) and isinstance(item.get("agent_id"), str)}
...
def _receipt(agent_id: str, proof: AgentReadinessProof, route: dict[str, Any], verifier_evidence_digest: str, verifier_provider: str, verifier_model: str | None) -> dict[str, object]:
...
def main() -> int:
    ...
    try:
        receipt = run_web_agent_live_certification(repository_root=repository_root, browser_summary_path=Path(browser_summary), output_dir=output_dir)
    except Exception as exc:
        raise SystemExit(f"WEB_AGENT_LIVE_CERTIFICATION=FAIL: {exc}") from exc
    print(f"WEB_AGENT_LIVE_CERTIFICATION=PASS verified={receipt['verified_agent_count']}/{receipt['target_agent_count']}")
    return 0
