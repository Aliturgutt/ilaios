...
class EngineeringChangePlan:
    ...
    def to_factory_request(
        ...
        *,
        ...
        """Create review-only canonical Software Factory input.

        Validation commands are intentionally empty here: untrusted model output
        cannot choose shell commands. Build/test/runtime validation remains a
        separately governed deterministic/runtime gate.
        """
...
def compile_engineering_change_plan(
    ...
    *,
    ...
    if set(document) != {"summary", "changes"}:
        raise EngineeringChangePlanError("change proposal has unknown or missing top-level fields")
...
def _strict_json_object(text: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or not all(isinstance(key, str) for key in value):
        raise EngineeringChangePlanError("engineering provider output must be a JSON object")
    return value
...
def _reject_obvious_unbounded_path(path: str) -> None:
    ...
    if normalized.startswith("/") or any(part in {"", ".", ".."} for part in parts):
        raise EngineeringChangePlanError("change path is not repository-relative and bounded")
    if ":" in parts[0]:
