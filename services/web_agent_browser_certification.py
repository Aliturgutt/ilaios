    ...
    *, summary_path: Path, expected_source_sha: str
    ...
    for raw_name in receipt_names:
        ...
        try:
            ...
            raise WebAgentBrowserCertificationError("BrowserQA receipt JSON is malformed") from exc
    ...
    if len(receipt_digests) < 6:
        raise WebAgentBrowserCertificationError("BrowserQA egress receipt coverage is incomplete")
...
def _validate_summary(document: dict[str, Any], expected_source_sha: str) -> None:
    ...
    if not isinstance(admissions, list) or len(admissions) != 5:
        raise WebAgentBrowserCertificationError("BrowserQA admission evidence coverage is incomplete")
    ...
    for item in admissions:
        ...
        if item.get("admission_proven") is not True or item.get("admission_decision") != "admitted":
            raise WebAgentBrowserCertificationError("BrowserQA admission was not proven")
    if admission_actions != set(_REQUIRED_ACTIONS):
        raise WebAgentBrowserCertificationError("BrowserQA admission action coverage drifted")

    actions = document.get("actions")
    if not isinstance(actions, dict) or set(actions) != set(_REQUIRED_ACTIONS):
        raise WebAgentBrowserCertificationError("BrowserQA action evidence coverage drifted")
    for action in _REQUIRED_ACTIONS:
        ...
        if not isinstance(boundary, str) or not boundary.startswith("sha256:"):
            raise WebAgentBrowserCertificationError(f"BrowserQA {action} boundary evidence is missing")
        ...
        if action in {"snapshot", "screenshot"}:
            ...
            if not isinstance(digest, str) or len(digest) != 64:
                raise WebAgentBrowserCertificationError(f"BrowserQA {action} digest is missing")
