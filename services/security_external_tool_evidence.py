...
from services.security_factory import SecurityFactory, SecurityFactoryError, SecurityFinding, SecurityReport, Severity
...
def retest_external_tool(*, before: SecurityReport, after: SecurityReport):
...
def _finding(tool: str, finding_id: str, severity: Severity, location: str, message: str, remediation: str, line: int = 0) -> SecurityFinding:
    return SecurityFinding(
        ...
        remediation=remediation or "review and remediate the scanner finding, then rerun the same bounded exercise",
    )
    ...
    for result in results:
        ...
        for item in result.get("Misconfigurations") or []:
            ...
            findings.append(_finding(
                ...
                str(item.get("Resolution") or "apply the reviewed Trivy configuration remediation"),
            ))
    return findings
...
def _nuclei(path: Path) -> list[SecurityFinding]:
    ...
    for raw in lines:
        ...
        findings.append(_finding(
            ...
            str(info.get("remediation") or "review the matched Nuclei template and remediate the affected surface"),
        ))
    return findings
...
def _zap(path: Path) -> list[SecurityFinding]:
    ...
    findings: list[SecurityFinding] = []
    risk = {"3": Severity.HIGH, "2": Severity.MEDIUM, "1": Severity.LOW, "0": Severity.INFO}
    for site in sites:
