...
The verifier is a P0 dependency, not a 22nd P0 target. It executes through the
same NamedAgentExecutor/GovernedRuntime/provider boundary and receives only a
hash/evidence envelope, not raw producer output. Its verdict must attest the
exact producer digest before it can satisfy an agent readiness gate.
"""
...
from services.runtime.ai_provider_adapter import AIProviderError
    ...
    def verify(
        ...
        *,
        ...
        invocation = AgentInvocation(
            ...
            # The envelope contains only canonical IDs/digests/metadata hashes;
            # no producer body, credential, or user secret is sent externally.
            dlp_approved=True,
...
def _parse_verdict(
    ...
    if document.get("producer_evidence_digest") != producer_digest:
        raise IndependentVerifierExecutionError(
            "verifier did not attest exact producer evidence"
        )
    ...
    if verdict == "PASS" and normalized:
        raise IndependentVerifierExecutionError(
            "PASS verdict cannot retain unresolved findings"
        )
