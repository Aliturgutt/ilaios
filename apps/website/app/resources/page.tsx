...
export default function Page(){return <>
  <section className="shell page-hero compact-page-hero resources-hero" style={{paddingTop:"30px",paddingBottom:"26px"}}><div className="eyebrow">Resources</div><h1>Engineering perspectives behind ILAIOS.</h1><p className="lead">Articles on product architecture, security and governed automation. Resources explains the thinking; Docs carries the technical reference.</p></section>
  <section className="section resources-insights" style={{paddingTop:"28px",paddingBottom:"32px"}}><div className="shell"><div className="compact-heading-row" style={{marginBottom:"20px"}}><div><div className="eyebrow">Latest perspectives</div><h2>Readable technical context without turning the page into documentation.</h2></div></div><div className="detail-directory">{insights.map(([kind,title,text,href])=><Link href={href} key={href}><span>{kind}</span><strong>{title}</strong><i>→</i><p>{text}</p></Link>)}</div></div></section>
  <section className="section compact-section"><div className="shell actions"><Link className="text-link" href="/docs">Technical documentation →</Link><Link className="text-link" href="/architecture">Architecture →</Link></div></section>
</>}
