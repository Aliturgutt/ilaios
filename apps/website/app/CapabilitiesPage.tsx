...
export default function CapabilitiesPage({ locale }: { locale: Locale }) {
  ...
  return <>
    ...
    <section className="section"><div className="shell"><div className="compact-heading-row"><div><h2>{c.capabilityTitle}</h2></div></div><div className="grid two-up capability-matrix">{c.capabilities.map(([title,text]) => <article className="card" key={title}><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
