...
const explore = [
  ["Platform", "See how experience, control, execution, and evidence are separated into durable system boundaries.", "/platform"],
  ["Control Plane", "Explore where policy, permissions, orchestration, validation, and authoritative state live.", "/platform/control-plane"],
  ["Security", "Review the trust model for permissions, approvals, fail-closed execution, audit context, and evidence.", "/security"],
] as const;
...
export default function Home() {
  return <>
    ...
    <section className="section"><div className="shell"><div className="eyebrow">Explore ILAIOS</div><h2>Go from the system model to its trust boundaries.</h2><div className="detail-link-grid">{explore.map(([title,text,href]) => <Link className="detail-link-card" href={href} key={href}><span>Explore</span><h3>{title}</h3><p>{text}</p><strong>Open detail →</strong></Link>)}</div></div></section>
  </>;
}
