...
export default function FactoriesPage({ locale }: { locale: Locale }) {
  ...
  return <>
    ...
    <section className="section"><div className="shell"><div className="section-heading"><div><div className="eyebrow">{c.eyebrow}</div><h2>{c.catalogTitle}</h2></div><p>{c.catalogLead}</p></div><div className="grid three-up factories-catalog-grid">{c.factories.map(([name, outcome, readiness, href], index) => <article className="card" key={href}><h3>{name}</h3><p>{outcome}</p><Link className="text-link" href={href}>{c.open} →</Link></article>)}</div></div></section>
    <section className="section surface-section factories-combine-section"><div className="shell compact-cta"><div><div className="eyebrow">{c.combineEyebrow}</div><h2>{c.combineTitle}</h2><p>{c.combineLead}</p></div><div className="actions"><Link className="button" href={`${base}/capabilities`}>{c.closePrimary}</Link><Link className="button secondary" href={`${base}/how-it-works`}>{c.closeSecondary}</Link></div></div></section>
  </>;
}
