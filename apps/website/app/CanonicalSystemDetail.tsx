...
const copy = {
  ...
  tr: {
    journey: {
      ...
      steps: ["Giriş yap", "Tenant + proje", "Doğal dil hedefi", "Niyet + gereksinimler", "Kabul ölçütleri", "Yetkili bağlam", "Sınırlandırılmış plan / DAG", "Yetenek + üretim alanı", "Yürütme kabulü", "Gerekirse onay", "Otonom yürütme", "Bağımsız kabul"],
    },
    runtime: {
      label: "Kabul, yönlendirme ve yürütme",
      title: "Yetenek, yönlendirme optimizasyonundan önce yetki sınırlarından geçer.",
      text: "Güvenlik, gizlilik, yerleşim, araç kapsamı, risk, kalite ve bütçe görevi sınırlar; ardından TEK RoutingDecision çalışan sınıfı, onaylı adaptör ve değiştirilebilir sağlayıcıyı seçer.",
      admission: ["Yetki", "Tenant izolasyonu", "Gizlilik / yerleşim", "DLP / gizli bilgiler", "Araç izni", "Risk", "Kalite tabanı", "Bütçe / kota"],
      execution: ["ExecutionGrant", "TEK RoutingDecision", "Kuyruk / zamanlayıcı", "Çalışan kiralaması + çitleme", "Çalışan", "Onaylı beceri", "Araç / sağlayıcı adaptörü", "Adım sonucu"],
    },
    knowledge: {
      label: "Yetkili bilgi katmanı",
      title: "Bilgi üretim alanlarını besler; üretim alanı veya yetki kaynağına dönüşmez.",
      text: "Bilgi getirme; kullanıcı, tenant, proje ve amaca göre yetkilendirilir; dönen her bilgi birimi kaynak kökenini korur ve tenant'lar arası sızıntı reddedilir.",
    ...
    cost: {
      label: "Yönlendirme içinde FinOps",
    ...
    maturity: {
      ...
      text: "Kamuya açık ürün dili; kanonik yönü, gözlemlenen uygulama, test, CI, çalışma zamanı ve dağıtım kanıtından ayrı tutar.",
      steps: ["DESIGNED", "SPECIFIED", "IMPLEMENTED", "TESTED", "VERIFIED", "DEPLOYED / PRODUCTION"],
    },
    web: {
      label: "Web Factory tam yaşam döngüsü",
      title: "Bitmiş web sitesi, tek bir üretim olayı değil; üretim zinciri ve kanıttır.",
      text: "Kanonik Web Factory hedefi; araştırma, tasarım, uygulama, tarayıcı ve kalite kapıları, sınırlı onarım ve dağıtım doğrulaması üzerinden taşır.",
      steps: ["Web sitesi hedefi", "Araştırma", "Bilgi mimarisi", "Metin", "Tasarım sistemi", "Görsel tasarım", "Uygulama", "Tarayıcı QA", "Güvenlik QA", "Erişilebilirlik", "Performans", "SEO", "Görsel QA", "Kabul", "Sınırlı onarım", "Dağıtım doğrulaması", "Bitmiş site + kanıt"],
    },
  },
} as const;
