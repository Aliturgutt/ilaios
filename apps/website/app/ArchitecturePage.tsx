...
const copy = {
  ...
  tr: {
    ...
    lead: "ILAIOS; ürün deneyimi ve yürütme kaynaklarını kimlik, politika, onaylar, durum, doğrulama, kanıt ve kurtarmanın sahibi olan arka uç otoritesinden ayrı tutar.",
    flowTitle: "Her katmanın tek görevi olduğunda sistem anlaşılır kalır.",
    layers: [["Deneyim", "Web, Masaüstü ve API yüzeyleri insanların iş talep etmesini, durumu incelemesini ve sonucu almasını sağlar."], ["Kontrol", "Kimlik, izinler, politika, onaylar ve kalıcı iş akışı durumu neyin olabileceğini belirler."], ["Yürütme", "Üretim alanları, yetenekler, araçlar ve sağlayıcılar yalnız kontrol katmanının kabul ettiği işi yapar."], ["Doğrulama", "Kontroller ve kabul ölçütleri üretilen işin ilerleyip ilerleyemeyeceğini belirler."], ["Kanıt ve kurtarma", "Önemli durum, kaynak kökeni ve sınırlandırılmış hata yönetimi sonuçları incelenebilir tutar."]],
...
export default function ArchitecturePage({ locale }: { locale: Locale }) {
  ...
  return <>
    <section className="section"><div className="shell architecture-primary"><div><div className="eyebrow">{locale === "tr" ? "Sistem katmanları" : "System layers"}</div><h2>{c.flowTitle}</h2><div className="architecture-layer-list">{c.layers.map(([title,text],index)=><article key={title}><span>{String(index+1).padStart(2,"0")}</span><strong>{title}</strong><p>{text}</p></article>)}</div></div><div className="architecture-right"><SpatialArchitecture locale={locale} /><aside className="architecture-hero-inset"><div className="eyebrow">{c.eyebrow}</div><h1>{c.title}</h1><p className="lead">{c.lead}</p><div className="actions"><Link className="button" href={`${base}/use-ilaios`}>{c.use}</Link></div></aside></div></div></section>
