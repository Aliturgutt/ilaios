...
export default function SecurityPage({ locale }: { locale: Locale }) {
  ...
  return <>
    ...
    <section className="section"><div className="shell security-boundary-layout"><div><div className="eyebrow">{locale === "tr" ? "Güven sınırı" : "Trust boundary"}</div><h2>{c.boundaryTitle}</h2><p>{c.boundaryText}</p><div className="security-detail-links"><Link href={`${base}/security/permissions`}>{c.permissions} →</Link><Link href={`${base}/security/approvals`}>{c.approvals} →</Link><Link href={`${base}/security/audit`}>{c.audit} →</Link></div></div><div className="trust-gate"><div className="dark-surface"><span>{locale === "tr" ? "İSTEMCİ" : "CLIENT"}</span><strong>{c.client}</strong></div><i><small>{c.contract}</small></i><div className="is-authority dark-surface"><span>{locale === "tr" ? "KONTROL KATMANI" : "CONTROL PLANE"}</span><strong>{c.core}</strong></div></div></div></section>
    ...
    <section className="section"><div className="shell"><div className="section-heading"><div><div className="eyebrow">{locale === "tr" ? "Yürütme öncesi" : "Before execution"}</div><h2>{c.admissionTitle}</h2></div><p>{c.admissionLead}</p></div><div className="security-process">{c.path.map(([n, title, detail]) => <article className="dark-surface" style={readableSurface} key={n}><span style={readableMuted}>{n}</span><strong style={readableSurface}>{title}</strong><small style={readableMuted}>{detail}</small></article>)}</div></div></section>
    <section className="section surface-section"><div className="shell"><div className="compact-heading-row"><div><div className="eyebrow">{locale === "tr" ? "Güvenlik ilkeleri" : "Security principles"}</div><h2>{c.pathTitle}</h2></div></div><div className="principle-directory">{c.principles.map(([title, text], index) => <article className="dark-surface" style={readableSurface} key={title}><span style={readableMuted}>{String(index + 1).padStart(2, "0")}</span><strong style={readableSurface}>{title}</strong><p style={readableMuted}>{text}</p></article>)}</div></div></section>
