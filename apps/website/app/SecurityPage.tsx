...
export default function SecurityPage({ locale }: { locale: Locale }) {
  ...
  return <>
    ...
    <section className="section"><div className="shell security-boundary-layout"><div><div className="eyebrow">{locale === "tr" ? "Güven sınırı" : "Trust boundary"}</div><h2>{c.boundaryTitle}</h2><p>{c.boundaryText}</p><div className="security-detail-links"><Link href={`${base}/security/permissions`}>{c.permissions} →</Link><Link href={`${base}/security/approvals`}>{c.approvals} →</Link><Link href={`${base}/security/audit`}>{c.audit} →</Link></div></div><div className="trust-gate"><div className="dark-surface"><span>CLIENT</span><strong>{c.client}</strong></div><i><small>{c.contract}</small></i><div className="is-authority dark-surface"><span>CONTROL PLANE</span><strong>{c.core}</strong></div></div></div></section>
