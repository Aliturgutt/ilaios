...
const copy = {
  en: {
    ...
    operatingStatus: "Canonical direction · In development",
  ...
  tr: {
    ...
    operatingStatus: "Kanonik yön · Geliştiriliyor",
...
export default function HomePage({ locale }: { locale: Locale }) {
  ...
  return <>
    <section className="home-hero shell" data-visual-role="home-hero"><div className="home-hero-copy"><div className="eyebrow">{c.eyebrow}</div><h1>{c.title}</h1><p className="lead">{c.lead}</p><div className="actions"><Link className="button" href={`${base}/use-ilaios`}>{c.primary}</Link><Link className="button secondary" href={`${base}/how-it-works`}>{c.secondary}</Link></div><SystemMotionSignature /></div><ProductExperience locale={locale} /></section>
    ...
    <section className="section surface-section"><div className="shell"><div className="section-heading"><div><div className="eyebrow">{c.operatingEyebrow}</div><h2>{c.operatingTitle}</h2></div><p>{c.operatingLead}</p></div><div className="grid two-up">{c.operating.map(([title,text]) => <article className="card" key={title}><h3>{title}</h3><p>{text}</p></article>)}</div><p className="muted">{c.operatingStatus}</p></div></section>
    <section className="section"><div className="shell"><div className="compact-heading-row"><div><div className="eyebrow">{c.processEyebrow}</div><h2>{c.processTitle}</h2></div></div><div className="process-rail" data-visual-role="five-step-execution">{c.process.map(([n, title, text]) => <article key={n}><span>{n}</span><strong>{title}</strong><p>{text}</p></article>)}</div></div></section>
    <section className="section surface-section"><div className="shell"><FactoryExplorer locale={locale} /></div></section>
    <section className="section"><div className="shell"><div className="section-heading"><div><div className="eyebrow">{c.goalEyebrow}</div><h2>{c.goalTitle}</h2></div><p>{c.goalLead}</p></div><div className="runtime-line">{c.goalFlow.map((step,index) => <div key={step}><span>{String(index + 1).padStart(2, "0")}</span><strong>{step}</strong></div>)}</div><p className="muted">{c.operatingStatus}</p></div></section>
    ...
    <section className="section evidence-section"><div className="shell evidence-story evidence-story-interactive"><div className="evidence-story-copy"><div className="eyebrow">{c.evidenceEyebrow}</div><h2>{c.evidenceTitle}</h2></div><GovernanceEvidence locale={locale} /></div></section>
