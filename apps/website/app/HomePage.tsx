...
const copy = {
  en: {
    ...
    outcomesEyebrow: "Beyond the examples",
    outcomesTitle: "Explore all nine production areas.",
    outcomesLead: "Explore an illustrative workflow above for each area. Web, video, software and applications are in preview; research and data, security, creative documents, commerce and growth, and personal operations are in development. Check each area page for its current public readiness.",
    outcomesAction: "Explore all nine production areas",
  ...
  tr: {
    ...
    outcomesEyebrow: "Örneklerin ötesinde",
    outcomesTitle: "Dokuz üretim alanının tamamını keşfet.",
    outcomesLead: "Yukarıda her alan için örnek iş akışını inceleyebilirsin. Web sitesi, video, yazılım ve uygulama önizleme aşamasındadır; araştırma ve veri, güvenlik, yaratıcı dokümanlar, ticaret ve büyüme ile kişisel operasyonlar geliştirilmektedir. Güncel kullanıma hazırlık durumunu her alanın sayfasından kontrol et.",
    outcomesAction: "Dokuz üretim alanının tamamını keşfet",
...
export default function HomePage({ locale }: { locale: Locale }) {
  ...
  return <>
    ...
    <section className="section surface-section"><div className="shell compact-cta"><div><div className="eyebrow">{c.outcomesEyebrow}</div><h2>{c.outcomesTitle}</h2><p>{c.outcomesLead}</p></div><div className="actions"><Link className="button secondary" href={`${base}/factories`}>{c.outcomesAction}</Link></div></div></section>
