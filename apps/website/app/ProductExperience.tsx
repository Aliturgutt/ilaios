...
const copy = {
  en: {
    label: "Example outcome", title: "One request can become finished work.", note: "Illustrative product flow. This preview does not execute providers or create production work.", request: "Example request", delivery: "What the finished path includes", checks: "Typical checks", open: "Explore this outcome",
    modes: [
  ...
  tr: {
    label: "Örnek sonuç", title: "Tek bir istek bitmiş işe dönüşebilir.", note: "Bu açıklayıcı bir ürün akışıdır. Buradaki önizleme sağlayıcı çalıştırmaz veya production işi oluşturmaz.", request: "Örnek istek", delivery: "Bitmiş yolun içerdiği sonuç", checks: "Tipik kontroller", open: "Bu sonucu keşfet",
    modes: [
...
export default function ProductExperience({ locale }: { locale: Locale }) {
  ...
  const moveTab = (next: number) => { const index = (next + c.modes.length) % c.modes.length; setModeIndex(index); tabRefs.current[index]?.focus(); };
  ...
  return <div className="product-experience" data-visual-role="illustrative-product-outcome">
    ...
    <div className="product-mode-tabs" role="tablist" aria-label={locale === "tr" ? "Sonuç türü" : "Outcome type"}>
      {c.modes.map((item, index) => <button key={item.key} ref={element => { tabRefs.current[index] = element; }} type="button" role="tab" aria-selected={modeIndex === index} tabIndex={modeIndex === index ? 0 : -1} className={modeIndex === index ? "is-active" : ""} onClick={() => setModeIndex(index)} onKeyDown={event => { if (event.key === "ArrowRight") { event.preventDefault(); moveTab(modeIndex + 1); } if (event.key === "ArrowLeft") { event.preventDefault(); moveTab(modeIndex - 1); } if (event.key === "Home") { event.preventDefault(); moveTab(0); } if (event.key === "End") { event.preventDefault(); moveTab(c.modes.length - 1); } }}>{item.label}</button>)}
    </div>
