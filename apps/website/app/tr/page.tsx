...
const explore = [
  ["Platform", "Deneyim, kontrol, yürütme ve kanıt katmanlarının kalıcı sistem sınırlarıyla nasıl ayrıldığını inceleyin.", "/tr/platform"],
  ["Kontrol Katmanı", "Politika, izinler, orkestrasyon, doğrulama ve yetkili durumun nerede yaşadığını görün.", "/tr/platform/control-plane"],
  ["Güvenlik", "İzinler, onaylar, fail-closed yürütme, audit bağlamı ve kanıt odaklı güven modelini inceleyin.", "/tr/security"],
] as const;
...
export default function Page() {
  return <>
    ...
    <section className="section"><div className="shell"><div className="eyebrow">ILAIOS'u inceleyin</div><h2>Sistem modelinden güven sınırlarına ilerleyin.</h2><div className="detail-link-grid">{explore.map(([title,text,href]) => <Link className="detail-link-card" href={href} key={href}><span>İncele</span><h3>{title}</h3><p>{text}</p><strong>Detayı aç →</strong></Link>)}</div></div></section>
  </>;
}
