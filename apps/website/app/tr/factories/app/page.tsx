...
export default function Page() { return <>
  <section className="shell page-hero"><div className="eyebrow">ILAIOS App Factory</div><h1>Ürün fikrinden sınırlandırılmış uygulama sonucuna; yayın yetkisi açık ve ayrı kalır.</h1><p className="lead">App Factory artık yalnız inceleme ve planlama kavramı değildir: depo kanıtı, Windows öncelikli sınırlandırılmış tamamlanmış ürün yolunu içerir. Bu, Android/iOS, üretim imzalama veya mağaza yayınının tamamlandığı anlamına gelmez.</p><div className="factory-availability-banner"><span className="availability-chip is-preview">Önizleme</span><p>Windows öncelikli sınırlandırılmış tamamlanmış ürün kanıtı depoda vardır. Android/iOS, imzalama, mağaza yayını, canlı kurulum ve farklı uygulama türlerinin tamamı için kapsam ayrı geçitler olarak kalır.</p></div></section>

  <section className="section surface-section factory-visual-section"><div className="shell"><div className="section-heading"><div><div className="eyebrow">App Factory hedef yaşam döngüsü</div><h2>Ürün fikirlerini mağazaya hazır hedeflere dönüştür; mağaza yayınını kanıtsız tamamlandı diye sunma.</h2></div><p>Sağlanan görsel hedef ürün yaşam döngüsünü anlatır. “Store Ready” mağaza hazırlığı hedefidir; imzalama, gönderim, sertifikasyon veya canlı kurulumun gerçekleştiğinin kanıtı değildir.</p></div><ThemedDiagram light="/visuals/app-light.avif" dark="/visuals/app-dark.avif" alt="ILAIOS App Factory diyagramı: istem ve referanslar, ürün ve UX tanımı, mimari, derleme, test ve doğrulama, iOS veya Android hazırlığı ve mağazaya hazır hedefi" caption="Hedef yaşam döngüsü: istem + referanslar → ürün/UX tanımı → mimari → derleme → test ve doğrulama → platform paketleme → mağaza hazırlığı. Güncel mobil/mağaza tamamlanması kanıta bağlı kalır." priority /></div></section>
