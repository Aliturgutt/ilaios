...
export default function Page() {
  return <>
    ...
    <section className="section surface-section factory-visual-section"><div className="shell"><div className="section-heading"><div><div className="eyebrow">Product flow</div><h2>Goal → governed work → production → verification → delivery.</h2></div><p>The diagram shows the user-facing product path. Internal provider and execution details remain behind the product boundary.</p></div><ThemedDiagram light="/visuals/general-flow-light.avif" dark="/visuals/general-flow-dark.avif" alt="ILAIOS public workflow from a stated goal through governed execution, production, verification and delivery" caption="Public product flow: the user defines the outcome; ILAIOS manages the governed work and verifies the result before delivery." priority /></div></section>
    <section className="section"><div className="shell split-copy"><div><div className="eyebrow">What verified means</div><h2>Verification answers a practical question: is this result ready to accept?</h2></div><div><p className="lead small">The exact checks depend on the work being produced. Required acceptance checks cannot be skipped just to call the work finished.</p><div className="verification-list">{verified.map(([title,text],i)=><div key={title}><span>{String(i+1).padStart(2,"0")}</span><strong>{title}</strong><small>{text}</small></div>)}</div></div></div></section>
