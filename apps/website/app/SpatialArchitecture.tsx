...
const copy: Record<Locale, { label: string; aria: string; hint: string; nodes: Node[] }> = {
  en: {
    ...
    nodes: [
      ...
      { title: "Policy", detail: "Authority boundary", description: "Applies identity, tenant isolation, permissions, privacy/residency, data-loss prevention/secrets, tool scope, risk, approval and budget boundaries before execution is admitted. Policy is authoritative; models are not." },
      { title: "Router", detail: "Capability selection", description: "Produces the single governed routing decision that selects an eligible capability, model, tool or provider inside the admitted scope. Routing may optimize execution but cannot widen permissions." },
  ...
  tr: {
    ...
    nodes: [
      { title: "Hedef", detail: "Kimliği doğrulanmış niyet", description: "Oturum açmış kullanıcının istediği sonucu; kiracı/proje bağlamı, kabul kriterleri ve yetkili bağlam ile birlikte sınırlandırılmış işe dönüştürür. Hedef sonucu tarif eder; kendi kendine yetki vermez." },
      { title: "Politika", detail: "Yetki sınırı", description: "Yürütme kabul edilmeden önce kimlik, kiracı izolasyonu, izinler, gizlilik/yerleşim, veri kaybı önleme/gizli bilgiler, araç kapsamı, risk, onay ve bütçe sınırlarını uygular. Yetki politikadadır; modelde değildir." },
      { title: "Yönlendirici", detail: "Yetenek seçimi", description: "Kabul edilmiş kapsam içinde uygun yetenek, model, araç veya sağlayıcıyı seçen tek yönetilen yönlendirme kararı üretir. Yönlendirme yürütmeyi optimize edebilir ancak izinleri genişletemez." },
      { title: "Üretim", detail: "Sınırlandırılmış yürütme", description: "Onaylı sınırlandırılmış iş akışını uygun yerleşik üretim alanı ve yönetilen çalışan/araç/sağlayıcı yolu üzerinden yürütür. Üretim alanları, alan yürütme yüzeyleridir; paralel kontrol düzlemleri değildir." },
      ...
      { title: "Kanıt", detail: "İncelenebilir kanıt", description: "Sonucun model iddialarına güvenmeden incelenebilmesi, denetlenebilmesi, kurtarılabilmesi ve açıklanabilmesi için doğrulama sonuçlarını, önemli olayları, kaynak kökenini ve yürütme soyunu korur." },
      { title: "Sonuç", detail: "Kabul edilmiş çıktı", description: "Yalnız gerekli kabul kapıları geçildikten sonra bitmiş ürünü temsil eder. Teslim veya dış yan etkiler yetkili kapsam içinde gerçekleşir; iş akışının izin verdiği yerde kontrol noktası/devam ve sınırlı onarım kullanılabilir." },
    ],
  },
};
...
export default function SpatialArchitecture({ locale, compact = false }: { locale: Locale; compact?: boolean }) {
  ...
  return <div ref={wrapperRef} className={`spatial-map ${compact ? "is-compact" : ""}`} data-visual-role="architecture-spatial-map">
    ...
    <div ref={stageRef} className="spatial-stage dark-surface" aria-label={c.aria} style={{ transform: "perspective(900px) rotateX(0deg) rotateY(0deg)" }} onPointerMove={event => { const rect = event.currentTarget.getBoundingClientRect(); tiltRef.current = { x: ((event.clientX - rect.left) / rect.width - 0.5) * 4, y: -((event.clientY - rect.top) / rect.height - 0.5) * 3 }; applyStageTransform(stageRef.current, tiltRef.current, scrollDepthRef.current); }} onPointerLeave={() => { tiltRef.current = { x: 0, y: 0 }; applyStageTransform(stageRef.current, tiltRef.current, scrollDepthRef.current); }}>
