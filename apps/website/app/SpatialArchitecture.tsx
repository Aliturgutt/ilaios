...
const copy: Record<Locale, { label: string; aria: string; hint: string; nodes: Node[] }> = {
  en: {
    ...
    nodes: [
      { title: "Goal", detail: "Authenticated intent", description: "Turns the signed-in user's requested outcome, kirac?/project context, acceptance criteria and authorized context into bounded work. The goal describes the outcome; it does not grant itself authority." },
      { title: "Policy", detail: "Authority boundary", description: "Applies identity, tenant isolation, permissions, privacy/residency, veri kayb? ?nleme/gizli bilgiler, tool scope, risk, approval and budget boundaries before execution is admitted. Policy is authoritative; models are not." },
      { title: "Router", detail: "Capability selection", description: "Produces the single governed y?nlendirme karar? that selects an eligible capability, model, tool or provider inside the admitted scope. Routing may optimize execution but cannot widen permissions." },
  ...
  tr: {
    ...
    nodes: [
      { title: "Hedef", detail: "Kimliği doğrulanmış niyet", description: "Oturum açmış kullanıcının istediği sonucu; kirac?/proje bağlamı, kabul kriterleri ve yetkili bağlam ile birlikte sınırlandırılmış işe dönüştürür. Hedef sonucu tarif eder; kendi kendine yetki vermez." },
      { title: "Politika", detail: "Yetki sınırı", description: "Yürütme kabul edilmeden önce kimlik, kirac? izolasyonu, izinler, gizlilik/yerle?im, veri kayb? ?nleme/gizli bilgiler, araç kapsamı, risk, onay ve bütçe sınırlarını uygular. Yetki politikadadır; modelde değildir." },
      { title: "Yönlendirici", detail: "Yetenek seçimi", description: "Kabul edilmiş kapsam içinde uygun yetenek, model, araç veya sağlayıcıyı seçen tek yönetilen y?nlendirme karar? üretir. Yönlendirme yürütmeyi optimize edebilir ancak izinleri genişletemez." },
      ...
      { title: "Sonuç", detail: "Kabul edilmiş çıktı", description: "Yalnız gerekli kabul kapıları geçildikten sonra bitmiş ürünü temsil eder. Teslim veya dış yan etkiler yetkili kapsam içinde gerçekleşir; iş akışının izin verdiği yerde kontrol noktas?/devam ve s?n?rl? onar?m kullanılabilir." },
    ],
  },
};
