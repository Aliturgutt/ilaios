...
ROUTES = (
    ("home", ""), ("about", "/about"), ("agents", "/agents"),
    ("architecture", "/architecture"), ("capabilities", "/capabilities"),
    ("contact", "/contact"), ("core", "/core"), ("desktop", "/desktop"),
    ("docs", "/docs"), ("enterprise", "/enterprise"), ("factories", "/factories"),
    ...
    ("factory-software", "/factories/software"), ("factory-video", "/factories/video"),
    ("factory-web", "/factories/web"), ("how-it-works", "/how-it-works"),
    ("use-ilaios", "/use-ilaios"), ("individuals", "/individuals"),
    ("platform", "/platform"), ("platform-control-plane", "/platform/control-plane"),
    ("platform-evidence", "/platform/evidence"),
    ("platform-execution", "/platform/execution"),
    ("platform-validation", "/platform/validation"), ("privacy", "/privacy"),
    ("resources", "/resources"),
    ...
    ("security", "/security"), ("security-approvals", "/security/approvals"),
    ("security-audit", "/security/audit"), ("security-permissions", "/security/permissions"),
    ("solutions", "/solutions"), ("terms", "/terms"), ("trust", "/trust"),
    ("updates", "/updates"),
)
...
def run_page_checks(
    page: Page,
    *, locale: str, route_name: str, path: str, viewport_name: str,
    width: int, height: int, theme: str, internal_links: set[str],
    ...
    if status >= 400 or status == 0: raise RuntimeError(f"HTTP {status}")
    if actual_theme != theme: raise RuntimeError(f"theme mismatch: expected {theme}, got {actual_theme}")
    if h1.count() != 1 or not h1.is_visible(): raise RuntimeError("exactly one visible main H1 is required")
    if overflow > 1: raise RuntimeError(f"horizontal overflow {overflow}px")
    if console_errors: raise RuntimeError(f"console errors: {console_errors[:3]}")
    if page_errors: raise RuntimeError(f"page errors: {page_errors[:3]}")
    if broken_images: raise RuntimeError(f"broken images: {broken_images[:3]}")
    if chromatic_ui: raise RuntimeError(f"chromatic UI outside canonical assets: {chromatic_ui[:3]}")
    ...
    record: dict[str, object] = {
        ...
        "broken_images": broken_images, "chromatic_ui": chromatic_ui,
    ...
    if viewport_name == "desktop":
        ...
        for href in page.locator("a[href]").evaluate_all("els => els.map((el)=>el.getAttribute('href')).filter(Boolean)"):
            ...
            if normalized is not None: internal_links.add(normalized)
    return record
...
def main() -> int:
    ...
    with sync_playwright() as playwright:
                ...
                for viewport_name, width, height in VIEWPORTS:
                    ...
                    try:
                        ...
                        if route_name in SCREENSHOT_ROUTE_NAMES and viewport_name in {"desktop", "mobile"}:
                            name=f"light__{locale}__{route_name}__{viewport_name}-{width}x{height}.png"; page.screenshot(path=str(screenshots/name), full_page=True); record["screenshot"]=f"screenshots/{name}"
                        record["result"]="PASS"
                        ...
                        records.append(record); page.close()
                ...
                for viewport_name, width, height in DARK_VIEWPORTS:
                    ...
                    try:
                        ...
                        if route_name in SCREENSHOT_ROUTE_NAMES:
                            name=f"dark__{locale}__{route_name}__{viewport_name}-{width}x{height}.png"; page.screenshot(path=str(screenshots/name), full_page=True); record["screenshot"]=f"screenshots/{name}"
                        record["result"]="PASS"
                    except Exception as exc:  # noqa: BLE001
        ...
        for target in sorted(internal_links):
            ...
            status=response.status; ok=status<400
            internal_link_results.append({"target":target,"status":status,"result":"PASS" if ok else "FAIL"})
            if not ok: failures.append(f"internal link {target}: HTTP {status}")

        context.close(); browser.close()

    report={
        "schema":"ilaios.website-v2.visual-qa.v5", "base_url":BASE_URL,
        "public_route_pairs":len(ROUTES), "localized_routes":len(ROUTES)*2,
        "light_viewports":[name for name,*_ in VIEWPORTS], "dark_viewports":[name for name,*_ in DARK_VIEWPORTS],
        "checks":len(records), "internal_link_targets":len(internal_link_results),
        "internal_link_results":internal_link_results, "header_baselines":header_baselines,
        "failures":failures, "status":"FAIL" if failures else "PASS", "records":records,
    }
