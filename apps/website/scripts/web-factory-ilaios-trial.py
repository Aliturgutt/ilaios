...
def _apply_page_architecture(project_root: Path) -> dict[str, object]:
    ...
    brand_render = '<a className="brand" href={`/${props.locale}`} aria-label="ILAIOS"><img className="brand-logo-light" src="/brand/13-ilaios-primary-horizontal-light.jpg" alt="ILAIOS" /><img className="brand-logo-dark" src="/brand/02-ilaios-primary-horizontal-dark.jpg" alt="" aria-hidden="true" /></a>'
...
const nav=[...document.querySelectorAll('header nav a')];const touch=[...document.querySelectorAll('header nav a,.primary-action,button')].filter(el=>{const r=el.getBoundingClientRect();return r.width>0&&r.height>0&&(r.width<44||r.height<44)}).length;
...
const sections=[...document.querySelectorAll('main > section')];let whitespace=0;let maxGap=0;for(let i=0;i<sections.length;i++){const r=sections[i].getBoundingClientRect();const s=getComputedStyle(sections[i]);const text=(sections[i].textContent||'').trim();if(r.height>innerHeight*1.45&&text.length<180)whitespace++;if(i>0){const prev=sections[i-1].getBoundingClientRect();maxGap=Math.max(maxGap,r.top-prev.bottom);}}
const brand=document.querySelector('.brand img');const primary=[...document.querySelectorAll('.primary-action')].filter(el=>el.getBoundingClientRect().width>0).length;
