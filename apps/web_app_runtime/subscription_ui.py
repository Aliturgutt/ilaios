...
def render_subscription(locale: str) -> bytes:
    ...
    def value(item: object) -> str:
        return escape(str(item)) if item is not None else copy("Not specified", "Henüz belirtilmedi")
    ...
    for plan in plans:
        ...
        price = (copy("Custom quote", "Özel teklif") if plan["price_kind"] == "custom" else
                 copy("Price pending", "TL fiyatı bekleniyor") if plan["monthly_price"] is None else
                 copy("Free", "Ücretsiz") if plan["monthly_price"] == 0 else
                 f'${plan["monthly_price"]} / {copy("month", "ay")}')
        parent = plan["parent"]
        inheritance = (copy("Start with included usage", "Dahil kullanımla başlayın") if parent is None else
                       f'{parent} '+copy("features included", "özellikleri dahil"))
        if plan["free_video_requires_verified_zero_cost"]:
            video = copy("Video only when a verified free option is available.", "Video yalnız doğrulanmış ücretsiz seçenek varsa sunulur.")
            ...
            video += copy("ceiling, subject to provider and policy support.", "üst sınırı, sağlayıcı ve politika desteğine bağlıdır.")
        else:
            video = f'{plan["video_pool_minutes"]} '+copy("min/month Mini 480p-equivalent shared pool", "dk/ay Mini 480p eşdeğeri ortak havuz")
            video += f' · {copy("Up to", "En fazla")} {plan["max_video_resolution"]}'
        button = (f'<a class="button" href="mailto:contact@ilaios.com">{copy("Request a quote", "Teklif iste")}</a>'
                  if plan["price_kind"] == "custom" else
                  f'<button class="button" type="button" data-plan="{plan_id}" data-price="{escape(price)}">{copy("View plan", "Planı incele")}</button>')
        cards.append(f'<article class="plan" aria-labelledby="plan-{plan_id}"><h3 id="plan-{plan_id}">{plan_id.title()}</h3>'
                     f'<p class="price">{escape(price)}</p><p class="muted">{inheritance}</p><p class="video">{video}</p>{button}</article>')
    ...
    for plan in plans:
        if plan["free_video_requires_verified_zero_cost"]:
            ...
            models = ", ".join(
                ...
                if name == "contract-allowlisted-models" else name for name in names
            )
        model_cells.append(f"<td>{escape(models)}</td>")
    rows.append(f'<tr><th scope="row">{copy("Video models", "Video modelleri")}</th>'+''.join(model_cells)+'</tr>')
    for key, en, turkish in [
        ...
        rows.append(f'<tr><th scope="row">{copy(en,turkish)}</th>'+''.join(f'<td>{value(p[key])}</td>' for p in plans)+'</tr>')
...
<nav aria-label="{copy('Preferences','Tercihler')}"><a href="?lang=tr" lang="tr" aria-current="{'page' if tr else 'false'}">TR</a>
<a href="?lang=en" lang="en" aria-current="{'false' if tr else 'page'}">EN</a>
<button id="subscription-theme" type="button" aria-pressed="false">{copy('Dark theme','Koyu tema')}</button></nav></header>
<main class="subscription-shell"><div class="eyebrow">ILAIOS / {copy('YOUR ACCOUNT','HESABINIZ')}</div>
<h1>{escape(title)}</h1><p class="lead">{copy('One account. Your included usage, across App and Desktop.','Tek hesap. App ve Desktop boyunca planınıza dahil kullanım.')}</p>
...
<p class="note">{copy('Payment is not available yet. No charge or automatic plan change will occur.','Ödeme henüz kullanılamıyor. Tahsilat veya otomatik plan değişikliği yapılmaz.')}</p></section>
...
<p class="note">{copy('Unspecified limits are not an unlimited-usage promise. Enterprise limits are subject to contract.','Belirtilmemiş limitler sınırsız kullanım vaadi değildir. Enterprise sınırları sözleşmeye bağlıdır.')}</p></section>
...
<footer><p>{copy('Seller','Satıcı')}: Ali Turgut · ILAIOS</p><a href="mailto:support@ilaios.com">{copy('Support','Destek')}</a> · <a href="mailto:privacy@ilaios.com">{copy('Privacy enquiries','Gizlilik talepleri')}</a> · <a href="mailto:contact@ilaios.com">{copy('Contact','İletişim')}</a></footer></main>
