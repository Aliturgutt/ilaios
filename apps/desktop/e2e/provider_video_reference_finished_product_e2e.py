...
def main() -> int:
    ...
    temporary = Path(tempfile.mkdtemp(prefix="ilaios-reference-provider-video-e2e-"))
    try:
...
def _run_reference_finished_product_acceptance(
    *, root: Path, proof_root: Path, api_key: str
    ...
    token = "desktop-reference-provider-video-e2e-token"
    ...
    objective = (
        ...
        "Use the attached product reference as the visual source of truth. Preserve the product's "
        "observable geometry, color relationships, distinctive illuminated feature, and emblem "
        "placement across the shot without inventing contradictory details. Present it as a "
        "premium studio reveal with realistic material response, controlled camera motion, subtle "
        "atmospheric depth, and clean audio. The result must be continuous visual footage, not "
        "title cards, presentation slides, static text panels, or generic motion graphics. "
    ...
    if prepared.get("execution_status") != "ADMITTED":
        raise RuntimeError(f"reference provider video request was not admitted: {prepared}")
    if prepared.get("adapter_id") != "video.product-runtime.v1":
        raise RuntimeError(f"wrong video adapter: {prepared}")

    reference_bytes = _reference_png_bytes()
    asset = references.put(
        ...
        instruction=(
            "Treat this product image as the visual source of truth. Preserve observable geometry, "
            "color palette, illuminated feature, and emblem placement."
        ),
        principal_id=principal_id,
        tenant_id=tenant_id,
    )
    ...
    if coordinator_state.get("execution_status") != "ACCEPTED":
        raise RuntimeError(
            f"reference coordinator did not reach ACCEPTED: {coordinator_state}"
        )

    bound = references.for_request(request_id)
    if len(bound) != 1 or bound[0].sha256 != asset.sha256:
        raise RuntimeError("immutable reference binding was not retained after execution")
    if raw_blob.exists():
        raise RuntimeError("successful reference execution did not release the raw image blob")

    frozen = ReferenceBriefCache(root / "reference-briefs.sqlite3").get(request_id)
    if frozen is None:
        raise RuntimeError("real multimodal reference analysis did not persist a frozen brief")
    if frozen.reference_sha256s != (asset.sha256,):
        raise RuntimeError("frozen reference brief digest does not match bound reference image")
    if not frozen.analyzer_id.startswith("openrouter-reference-analysis:"):
        raise RuntimeError(f"unexpected reference analyzer identity: {frozen.analyzer_id}")
    if not frozen.text.strip():
        raise RuntimeError("frozen reference visual brief is empty")
    ...
    if not rendered.is_file() or rendered.stat().st_size <= 100_000:
        raise RuntimeError("verified reference-provider MP4 delivery is missing or too small")
    ...
    if hashlib.sha256(rendered.read_bytes()).hexdigest() != artifact_digest:
        raise RuntimeError("persisted reference-provider video changed after acceptance")
    ...
    if video_stream is None or audio_stream is None:
        raise RuntimeError("reference-provider finished product lacks audio/video streams")
    ...
    if int(str(video_stream.get("width"))) != 1920 or int(
        str(video_stream.get("height"))
    ) != 1080:
        raise RuntimeError("reference-provider finished product is not 1920x1080")
    if not 3.0 <= float(probe.duration_seconds) <= 5.0:
        raise RuntimeError(
            "reference-provider duration is outside 4s tolerance: "
    ...
    if qa.get("semantic_passed") is not True:
        raise RuntimeError(f"reference semantic acceptance is not proven: {qa}")
    if qa.get("technical_passed") is not True:
        raise RuntimeError(f"reference technical acceptance is not proven: {qa}")
    ...
    terminal_evidence = {
        job_id: dict(value) for job_id, value in poller.terminal_evidence.items()
    }
    if len(terminal_evidence) != 1:
        raise RuntimeError(
            "expected terminal zero-cost evidence for 1 reference-provider job, "
            f"got {len(terminal_evidence)}"
        )
    costs = [float(value["cost"]) for value in terminal_evidence.values()]
    if any(cost != 0.0 for cost in costs):
        raise RuntimeError(f"reference provider cost evidence is not zero: {costs}")
    generation_ids = [
        str(value["generation_id"])
        for value in terminal_evidence.values()
    ...
    sources = sorted({str(value["source"]) for value in terminal_evidence.values()})
    _write_provider_evidence(proof_root, poller)
    ...
    receipt = {
        ...
        "provider_model": os.environ.get(
            "ILAIOS_VIDEO_MODEL_ID", SEEDANCE_FREE_MODEL_ID
        ),
...
def _reference_png_bytes(width: int = 640, height: int = 360) -> bytes:
    if width != 640 or height != 360:
        raise ValueError("reference certification raster is fixed at 640x360")
