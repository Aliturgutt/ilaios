...
void main() {
  testWidgets('factory cards pin the selected execution intent before submit', (
    ...
    expect(
      find.byKey(const ValueKey('factory-preset-software')),
      findsOneWidget,
    );
  ...
  testWidgets('settings quick actions are interactive', (
    ...
    expect(
      find.byKey(const Key('settings-notifications-action')),
      findsOneWidget,
    );
    ...
    expect(
      find.byKey(const Key('settings-diagnostics-action')),
      findsOneWidget,
    );
  ...
  testWidgets(
    'Workflows keeps the V4 reference hierarchy with contextual detail',
    (WidgetTester tester) async {
      ...
      expect(
        find.byKey(const ValueKey('workflow-row-wf-v4-1')),
        findsOneWidget,
      );
  ...
  testWidgets('verified deliveries expose local-delete without deleting evidence', (
    ...
    // Deliveries intentionally hide authoritative records while disconnected.
    // Supply an authorized connected projection to exercise local-copy actions.
    await tester.pumpWidget(
      IlaiosDesktopApp(
        locale: IlaiosLocale.english,
        operationalSnapshot: snapshot,
