...
void main() {
  testWidgets(
    'connected control plane without runtime data never invents canonical Home telemetry',
    (WidgetTester tester) async {
      ...
      // Home intentionally contains no agent runtime panel.
      expect(find.text('No verified runtime agent data'), findsNothing);
  ...
  testWidgets(
    'canonical Home projects only authority-derived agent runtime values',
    (WidgetTester tester) async {
      await tester.binding.setSurfaceSize(const Size(1600, 900));
      addTearDown(() => tester.binding.setSurfaceSize(null));
      ...
      expect(find.byKey(const Key('command-center-home')), findsOneWidget);
      // Runtime details are shown on Agents, never duplicated on Home.
