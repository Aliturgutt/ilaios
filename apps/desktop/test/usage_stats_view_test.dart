...
void main() {
  ...
  test('UsageStatsModel aggregates only observed runtime-route telemetry', () {
    ...
    expect(model.providers.map((row) => row.label), <String>['openrouter', 'google']);
    expect(model.providers.first.routes, 2);
    expect(model.models.map((row) => row.label), <String>['model-a', 'model-b']);
  ...
  testWidgets('Usage & Stats empty state keeps unavailable telemetry explicit', (
    ...
    expect(find.textContaining('No synthetic token, cost, provider, model, latency or success-rate values are generated'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
}
