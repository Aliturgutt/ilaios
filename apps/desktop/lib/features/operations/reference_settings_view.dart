...
import '../../app/desktop_text_scale.dart';
...
// Stable category accents shared by the settings tiles, navigation and details.
Color _settingsIconColor(IconData icon) {
  if (icon == Icons.check_circle_outline) return const Color(0xFF168650);
  if (icon == Icons.warning_amber_rounded) return const Color(0xFFC58B00);
  if (icon == Icons.settings_outlined) return const Color(0xFF4C9AFF);
  if (icon == Icons.palette_outlined) return const Color(0xFFAA77FF);
  if (icon == Icons.notifications_none_outlined) return const Color(0xFFFFB547);
  if (icon == Icons.shield_outlined ||
      icon == Icons.verified_user_outlined ||
      icon == Icons.policy_outlined) {
    return const Color(0xFF38C6A0);
  }
  if (icon == Icons.cloud_outlined) return const Color(0xFF38BDF8);
  if (icon == Icons.extension_outlined) return const Color(0xFFFF8A65);
  if (icon == Icons.person_outline) return const Color(0xFF60A5FA);
  if (icon == Icons.lock_outline) return const Color(0xFFF472B6);
  if (icon == Icons.tune_outlined) return const Color(0xFFB5C25B);
  if (icon == Icons.history) return const Color(0xFF60A5FA);
  if (icon == Icons.dns_outlined) return const Color(0xFF38BDF8);
  if (icon == Icons.apartment_outlined) return const Color(0xFF9362D6);
  if (icon == Icons.account_circle_outlined) return const Color(0xFF168650);
  return const Color(0xFF8FA9D6);
}
...
class _ReferenceSettingsViewState extends State<ReferenceSettingsView> {
  ...
  double _fontScale = desktopTextScale.value;
  ...
  @override
  void didUpdateWidget(covariant ReferenceSettingsView oldWidget) {
  ...
  void _setTheme(ThemeMode mode) {
    ...
    _record('Theme preview changed', 'Tema önizlemesi değiştirildi');
  }
  ...
  void _reset() {
    setState(() {
      ...
      _animations = true;
      _fontScale = 1;
      desktopTextScale.value = 1;
      if (kReleaseMode) unawaited(DesktopTextScaleStore.save(1));
      _previewTheme = widget.themeMode;
      _message = null;
    });
    _record(
      'Appearance defaults restored',
      'Görünüm varsayılanları geri yüklendi',
    );
  }
                    ...
                    ? _AppearancePanel(
                        ...
                        onCompact: (value) {
                          ...
                          _record(
                            ...
                            'Kompakt mod güncellendi',
                          );
                        },
                        onDense: (value) {
                          setState(() => _dense = value);
                          _record(
                            'Dense table mode updated',
                            'Yoğun tablo modu güncellendi',
                          );
                        },
                        onSidebar: (value) {
                          ...
                          _record(
                            ...
                            'Kenar çubuğu önizlemesi güncellendi',
                          );
                        },
                        onAnimations: (value) {
                          ...
                          _record(
                            ...
                            'Animasyon tercihi güncellendi',
                          );
                        },
                        onScale: (value) {
                          desktopTextScale.value = value;
                          setState(() => _fontScale = value);
                        },
                        onScaleEnd: (value) {
                          if (kReleaseMode) {
                            unawaited(DesktopTextScaleStore.save(value));
                          }
                          _record(
                            'Font scale updated',
                            'Yazı ölçeği güncellendi',
                          );
                        },
                      )
                    : _AuthorityPanel(
...
class _Header extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SizedBox(
    height: MediaQuery.textScalerOf(context).scale(54),
    child: Column(
  ...
  Widget build(BuildContext context) => Material(
    ...
    child: InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            Icon(icon, size: 23, color: _settingsIconColor(icon)),
                      ...
                      child: Row(
                        children: [
                          Icon(
                            icons[index],
                            size: 14,
                            color: _settingsIconColor(icons[index]),
                          ),
                          const SizedBox(width: 7),
                          Expanded(
                            child: Text(
...
class _SettingRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    constraints: BoxConstraints(
      minHeight: MediaQuery.textScalerOf(context).scale(35),
    ),
    decoration: BoxDecoration(
      border: Border(
...
class _SwitchRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ConstrainedBox(
    constraints: BoxConstraints(
      minHeight: MediaQuery.textScalerOf(context).scale(42),
    ),
    child: Row(
      children: [
        Expanded(
                  ...
                  child: Column(
                    children: [
                      if (!sidebarCollapsed)
                        ...
                        const Icon(Icons.apps_rounded, size: 15),
                      const SizedBox(height: 7),
                      for (var i = 0; i < 7; i++)
                        Container(
                          margin: EdgeInsets.only(bottom: dense ? 3 : 5),
                          height: 4,
                          width: sidebarCollapsed ? 11 : 48,
                          decoration: BoxDecoration(
                            color: i == 1
...
class _AuthorityPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final identityValues = <(IconData, String, String)>[
      (
        Icons.dns_outlined,
      ...
      (
        ...
        userSession?.tenantId ??
            _copy(context, 'Unavailable', 'Kullanılamıyor'),
      ),
      (
    ...
    final values = switch (section) {
      0 => <(IconData, String, String)>[identityValues[0], identityValues[2]],
      2 => <(IconData, String, String)>[identityValues[1], identityValues[3]],
      3 => <(IconData, String, String)>[identityValues[0]],
      4 => <(IconData, String, String)>[
        (
          Icons.cloud_outlined,
          _copy(context, 'Provider connection', 'Sağlayıcı bağlantısı'),
          providers.isEmpty
              ? _copy(context, 'Not configured', 'Yapılandırılmadı')
              : _copy(
                  context,
                  'Provider information available',
                  'Sağlayıcı bilgisi mevcut',
                ),
        ),
      ],
      5 => <(IconData, String, String)>[
        (
          Icons.extension_outlined,
          _copy(context, 'Integration status', 'Entegrasyon durumu'),
          _copy(
            context,
            'No verified integration data',
            'Doğrulanmış entegrasyon verisi yok',
          ),
        ),
      ],
      6 => <(IconData, String, String)>[
        (
          Icons.notifications_outlined,
          _copy(context, 'Notification status', 'Bildirim durumu'),
          _copy(
            context,
            'No verified notification data',
            'Doğrulanmış bildirim verisi yok',
          ),
        ),
      ],
      7 => <(IconData, String, String)>[
        (
          Icons.lock_outline,
          _copy(context, 'Privacy status', 'Gizlilik durumu'),
          _copy(
            context,
            'No verified privacy data',
            'Doğrulanmış gizlilik verisi yok',
          ),
        ),
      ],
      8 => <(IconData, String, String)>[
        (
          Icons.tune_outlined,
          _copy(context, 'Advanced settings', 'Gelişmiş ayarlar'),
          _copy(
            context,
            'No verified configuration data',
            'Doğrulanmış yapılandırma verisi yok',
          ),
        ),
      ],
      _ => identityValues,
                  ...
                  child: Row(
                    children: [
                      Icon(
                        value.$1,
                        size: 18,
                        color: _settingsIconColor(value.$1),
                      ),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      ...
      children: [
        Row(
          children: [
            Icon(icon, size: 13, color: _settingsIconColor(icon)),
            const SizedBox(width: 5),
            Expanded(
              child: Text(
                title,
...
class _StatusLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
    children: [
      Icon(icon, size: 15, color: _settingsIconColor(icon)),
      const SizedBox(width: 5),
      Expanded(
        child: Text(
          title,
