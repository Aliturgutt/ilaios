                ...
                for (var index = 0; index < _roles.length; index++) ...[
                  Expanded(
                    child: _AgentCard(
                      role: _roles[index].$1,
                      icon: _roles[index].$2,
                      lease: model.leaseFor(_roles[index].$1),
                      accent: index == _roles.length - 1
                          ? IlaiosTheme.coreBlue
                          : IlaiosTheme.enterpriseCyan,
                    ),
                  ),
                  if (index < _roles.length - 1) const SizedBox(width: 6),
                ],
              ],
            ),
          ),
                        ...
                        for (final tab in _tabs)
                          Expanded(
                            child: Material(
                              ...
                              child: InkWell(
                                key: ValueKey(
                                  'workspace-tab-${_normalize(tab).replaceAll(' ', '-')}',
                                ),
                        ...
                        child: Column(
                          ...
                          children: [
                            Icon(
                              icon,
                              size: 27,
                              color: IlaiosTheme.coreBlue.withValues(alpha: .8),
                            ),
                            const SizedBox(height: 6),
                            Text(
                              emptyTitle,
...
class _EvidenceList extends StatelessWidget {
  const _EvidenceList({required this.title, required this.records});
  ...
  Widget build(BuildContext context) => _InnerListShell(
        ...
        child: records.isEmpty
            ? Center(
                child: Text(
                  _isTr(context)
                      ? 'Dosya/çıktı projeksiyonu kullanılamıyor.'
                      : 'No file/artifact projection is available.',
                ),
              )
            : ListView.separated(
                padding: const EdgeInsets.all(10),
                ...
                itemBuilder: (context, index) {
                  ...
                  return Text(
                    '${record.action}  ·  ${record.executionId}',
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                  );
                },
...
class _EventList extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _InnerListShell(
        ...
        child: events.isEmpty
            ? Center(
                child: Text(_home(context, 'No live event records available.')),
              )
            : ListView.separated(
                padding: const EdgeInsets.all(10),
                ...
                itemBuilder: (context, index) {
                  ...
                  final label = _text(
                        event,
                        const ['event_type', 'type', 'status', 'message'],
                      ) ??
                      _home(context, 'Unavailable');
                  return Text(
                    label,
...
class _LatestLogs extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final events = model.snapshot.liveEvents;
...
String _roleKey(String value) => _normalize(value)
    .replaceAll('agent', '')
    .replaceAll('dev', '')
    .replaceAll('engineer', '')
    .trim();
