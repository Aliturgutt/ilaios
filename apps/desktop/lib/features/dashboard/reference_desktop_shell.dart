...
class _ReferenceDesktopShellState extends State<ReferenceDesktopShell> {
  ...
  Widget _buildSection() => switch (_section) {
        DesktopSection.home => ReferenceHomeDashboardView(
            projection: widget.projection,
            snapshot: widget.operationalSnapshot,
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Material(
        ...
        child: InkWell(
          key: ValueKey('reference-nav-${section.name}'),
          onTap: onTap,
          child: SizedBox(
            height: 42,
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  if (section == DesktopSection.liveWorkspace)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: IlaiosTheme.enterpriseCyan.withValues(alpha: .14),
...
class _ReferenceTenantCard extends StatelessWidget {
  const _ReferenceTenantCard({
    required this.snapshot,
    required this.userSession,
  });
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(
          children: [
            ...
            const Spacer(),
            _ReferenceSearchBox(onNavigate: onNavigate),
            const SizedBox(width: 18),
            _ReferenceIconButton(
              icon: Icons.notifications_none_rounded,
              tooltip: _isTr(context) ? 'Bildirimler' : 'Notifications',
              onTap: () => _showReferenceNotice(context),
            ),
            const SizedBox(width: 9),
            _ReferenceLanguageButton(),
            const SizedBox(width: 9),
...
class _ReferenceSearchBox extends StatelessWidget {
  const _ReferenceSearchBox({required this.onNavigate});
  ...
  Widget build(BuildContext context) => SizedBox(
        ...
        child: OutlinedButton(
          onPressed: () => _showReferenceCommandPalette(context, onNavigate),
...
class _ReferenceLanguageButton extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
...
class _ReferenceThemeButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return IconButton(
      key: const Key('reference-theme-toggle'),
      tooltip: light
...
class _ReferenceProfile extends StatelessWidget {
  const _ReferenceProfile({required this.userSession, required this.onTap});
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              const SizedBox(width: 7),
              if (label == context.tr('shell.systemHealth')) ...[
...
Future<void> _showReferenceCommandPalette(
...
Future<void> _showReferenceNotice(BuildContext context) => showDialog<void>(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: Text(
          _isTr(context)
              ? 'Yetkili çalışma zamanı bildirim verisi sunmadı. Tasarım amacıyla sentetik bildirim üretilmez.'
              : 'The authoritative runtime exposes no notification data. Synthetic notifications are not fabricated for visual parity.',
        ),
        actions: [
          TextButton(
