...
def _private_dns(*args: Any, **kwargs: Any) -> list[tuple[Any, ...]]:
    return [
...
def test_injected_transport_rejects_private_dns_before_call(
    ...
    monkeypatch.setattr(preview_probe, "getaddrinfo", _private_dns)
...
def test_injected_transport_revalidates_final_hostname_dns(
    ...
    def _resolve(host: str, port: int, **kwargs: Any) -> list[tuple[Any, ...]]:
