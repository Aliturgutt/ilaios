...
def test_video_acceptance_closes_resources_and_preserves_identity(tmp_path: Path) -> None:
    fixture = _Fixture(tmp_path)
    principal_id = "oidc|subject:user@example.test"
    tenant_id = "tenant/global-example"
    _prepare_video(
    ...
    state = fixture.coordinator.get("exec-video-2")
    ...
    grant_state = fixture.grants.state()
    ...
    assert revoked[0]["grant_id"] == grants[0]["grant_id"]
...
def test_failure_closes_product_execution_and_releases_resources(tmp_path: Path) -> None:
    fixture = _Fixture(tmp_path)
    _prepare_video(fixture, "exec-video-fail")
    fixture.grants.kill("worker-video", now=_now())
    with pytest.raises(GrantError, match="stopped"):
        fixture.coordinator.resume(
            "exec-video-fail", token="token", now=_now() + timedelta(seconds=1)
        )
    state = fixture.coordinator.get("exec-video-fail")
...
def test_stale_executing_request_closes_subordinate_resources(tmp_path: Path) -> None:
    fixture = _Fixture(tmp_path)
    prepared_at = _now()
    _prepare_video(fixture, "exec-video-stale", now=prepared_at)
    ...
    reconciled = fixture.coordinator.recover_stale(
        token="token", now=prepared_at + timedelta(minutes=16)
    )
    assert reconciled == (
        {"request_id": "exec-video-stale", "status": "INTERRUPTED"},
    )
    state = fixture.coordinator.get("exec-video-stale")
    assert state["execution_status"] == "INTERRUPTED"
    assert state["terminal"] is True
    assert "subordinate resources closed" in cast(str, state["terminal_reason"])
    assert fixture.scheduler.state()["leases"] == []
...
def test_fresh_executing_request_is_not_falsely_closed(tmp_path: Path) -> None:
    fixture = _Fixture(tmp_path)
    _prepare_video(fixture, "exec-video-active")
    with sqlite3.connect(fixture.coordinator_path) as connection:
        connection.execute(
            "UPDATE execution_requests SET status = 'EXECUTING', updated_at = ? "
            "WHERE request_id = ?",
            (_now().isoformat(), "exec-video-active"),
        )
    with pytest.raises(ExecutionCoordinatorError, match="already executing"):
        fixture.coordinator.resume(
            "exec-video-active", token="token", now=_now() + timedelta(minutes=1)
        )
    state = fixture.coordinator.get("exec-video-active")
    assert state["execution_status"] == "EXECUTING"
    assert state["terminal"] is False


def test_prepare_is_idempotent_and_conflicting_replay_is_rejected(tmp_path: Path) -> None:
    fixture = _Fixture(tmp_path)
    first = _prepare_video(fixture, "exec-idempotent-1")
    second = _prepare_video(fixture, "exec-idempotent-1")
    ...
    with pytest.raises(ExecutionCoordinatorError, match="conflicts"):
        _prepare_video(
            fixture,
            "exec-idempotent-1",
            objective="Create a different launch video and final MP4",
        )


def test_cancellation_is_idempotent_and_closes_product_runtime(tmp_path: Path) -> None:
    fixture = _Fixture(tmp_path)
    ...
    product = fixture.product.get_state("exec-cancel-1")
    assert product["status"] == "interrupted"
    assert product["terminal"] is True
    assert fixture.coordinator.get("exec-cancel-1")["terminal"] is True
