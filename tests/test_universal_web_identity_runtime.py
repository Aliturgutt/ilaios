...
class _LifetimeRejectingMicrosoftOAuth(_MicrosoftOAuth):
    def complete(
        self,
        *,
        state: str,
...
def test_microsoft_id_token_lifetime_failure_logs_specific_safe_reason(
    tmp_path: Path,
    caplog: pytest.LogCaptureFixture,
) -> None:
    runtime = _runtime(
        tmp_path / "identity.db",
        microsoft=_LifetimeRejectingMicrosoftOAuth(),
    )
    logger = logging.getLogger("apps.web_app_runtime.server")
    with caplog.at_level(logging.WARNING, logger=logger.name):
        response = runtime.dispatch(
