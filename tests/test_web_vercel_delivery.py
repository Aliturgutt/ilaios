...
class _FakeVercelTransport:
    def __init__(
        ...
        *,
        ...
        probe_url: str = "https://customer-site.vercel.app/",
    ) -> None:
    ...
    def probe(self, url: str) -> tuple[int, str]:
        ...
        return self.probe_status, self.probe_url
...
def _adapter(transport: _FakeVercelTransport) -> VercelWebDeploymentAdapter:
...
def test_vercel_deploy_binds_provenance_promotes_and_probes_https(
    tmp_path: Path,
) -> None:
...
def test_vercel_deploy_requires_authorization_before_credential_or_network(
    ...
    with pytest.raises(WebDeploymentError, match="authorization is not proven"):
        adapter.deploy(
            root,
...
def test_vercel_deploy_rejects_environment_secret_files(tmp_path: Path) -> None:
    root = _project(tmp_path)
...
def test_vercel_rollback_reconciles_target_provenance_and_health() -> None:
    ...
    transport = _FakeVercelTransport(
        ...
        probe_url="https://restored.vercel.app/",
    )

    receipt = _adapter(transport).rollback(
        deployment_id,
        source_commit_sha=source_sha,
        expected_artifact_sha256=artifact_sha,
