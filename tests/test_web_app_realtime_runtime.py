...
def test_publish_and_reconnect_replay_use_monotonic_sequences() -> None:
    ...
    first = realtime.publish(
        principal=_principal(), resource_type="Goal", resource_id="goal-1",
        event_type="created", payload={"status": "open"}, now=NOW, resource_version=1,
    )
    second = realtime.publish(
        principal=_principal(), resource_type="Goal", resource_id="goal-1",
        event_type="updated", payload={"status": "done"}, now=NOW, resource_version=2,
    )
    ...
    batch = realtime.subscribe(
        principal=_principal(), resource_type="Goal", resource_id="goal-1",
        after_sequence=first.sequence, now=NOW,
    )
...
def test_subscription_is_tenant_scoped() -> None:
    realtime = _runtime()
    realtime.publish(
        principal=_principal(tenant_id="tenant-1"), resource_type="Goal",
        resource_id="goal-1", event_type="state_changed",
        payload={"state": "Executing"}, now=NOW,
    )
    batch = realtime.subscribe(
        principal=_principal(tenant_id="tenant-2"), resource_type="Goal", now=NOW,
    )
    assert batch.events == ()
...
def test_default_deny_is_inherited_for_publish_and_subscribe() -> None:
    ...
    with pytest.raises(IdentityError, match="deny by default"):
        realtime.publish(
            principal=_principal(role="Unknown"), resource_type="Goal",
            resource_id="goal-1", event_type="created", payload={}, now=NOW,
        )
    with pytest.raises(IdentityError, match="deny by default"):
        realtime.subscribe(
            principal=_principal(role="Unknown"), resource_type="Goal", now=NOW,
        )
...
def test_stale_cursor_fails_closed_after_history_eviction() -> None:
    ...
    for index in range(3):
        realtime.publish(
            principal=_principal(), resource_type="Goal", resource_id=f"goal-{index}",
            event_type="updated", payload={"index": index}, now=NOW,
        )
    with pytest.raises(WebAppRealtimeRuntimeError) as exc:
        realtime.subscribe(
            principal=_principal(), resource_type="Goal", after_sequence=0, now=NOW,
        )
...
def test_batch_limit_reports_more_without_dropping_reconnect_cursor_truth() -> None:
    ...
    for index in range(3):
        realtime.publish(
            principal=_principal(), resource_type="Goal", resource_id=f"goal-{index}",
            event_type="created", payload={"index": index}, now=NOW,
        )
    batch = realtime.subscribe(
...
def test_invalid_payload_cursor_and_resource_version_fail_closed() -> None:
    ...
    with pytest.raises(WebAppRealtimeRuntimeError) as payload:
        realtime.publish(
            principal=_principal(), resource_type="Goal", resource_id="goal-1",
            event_type="created", payload={"bad": object()}, now=NOW,
        )
    assert payload.value.code == "INVALID_PAYLOAD"
    with pytest.raises(WebAppRealtimeRuntimeError) as version:
        realtime.publish(
            principal=_principal(), resource_type="Goal", resource_id="goal-1",
            event_type="created", payload={}, now=NOW, resource_version=0,
        )
