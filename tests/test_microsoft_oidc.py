...
CONSUMER_ISSUER = (
    f"https://login.microsoftonline.com/{MICROSOFT_CONSUMER_TENANT_ID}/v2.0"
)
...
def _environment() -> dict[str, str]:
    return {
        ...
        "ILAIOS_MICROSOFT_PRODUCTION_WEB_REDIRECTS": (
            "https://app.ilaios.com/auth/microsoft/callback,"
            "https://ilaios.com/auth/microsoft/callback"
        ),
    }


def _claims(
    *, issuer: str, subject: str = "microsoft-sub-123"
) -> VerifiedOIDCClaims:
...
def test_microsoft_environment_requires_distinct_dev_and_prod_clients() -> None:
    env = _environment()
    env["ILAIOS_MICROSOFT_DEVELOPMENT_WEB_CLIENT_ID"] = env[
        "ILAIOS_MICROSOFT_PRODUCTION_WEB_CLIENT_ID"
    ]
...
def test_microsoft_production_redirect_allowlist_rejects_loopback_and_http() -> None:
    for redirect in (
        "http://app.ilaios.com/auth/microsoft/callback",
        "https://127.0.0.1/auth/microsoft/callback",
    ):
        env = _environment()
...
def test_microsoft_desktop_provider_uses_common_authority_without_secret() -> None:
    provider = MicrosoftOIDCEnvironment.from_environment(
        _environment()
    ).desktop_provider()
...
def test_microsoft_identity_key_is_namespaced_by_verified_issuer() -> None:
    personal = verified_microsoft_identity(
        _claims(issuer=CONSUMER_ISSUER, subject="same-sub")
    )
    organization = verified_microsoft_identity(
        _claims(issuer=ORG_ISSUER, subject="same-sub")
    )
