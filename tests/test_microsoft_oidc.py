...
from services.microsoft_oidc import MICROSOFT_AUTHORIZATION_ENDPOINT, MICROSOFT_CONSUMER_TENANT_ID, MICROSOFT_JWKS_URI, MICROSOFT_TOKEN_ENDPOINT, MicrosoftOIDCConfigurationError, MicrosoftOIDCEnvironment, verified_microsoft_identity
...
def _claims(*, issuer: str, subject: str = "microsoft-sub-123") -> VerifiedOIDCClaims:
    return VerifiedOIDCClaims(issuer=issuer, audience="prod-ms-client", subject=subject, tenant_id="provider-derived-not-canonical", expires_at=NOW + timedelta(minutes=30), issued_at=NOW - timedelta(minutes=1), kind=IdentityKind.HUMAN, roles=frozenset({"user"}), attributes=frozenset({("preferred_username", "User@Example.com")}), authentication_methods=frozenset({"pwd"}))


def test_microsoft_environment_requires_distinct_dev_and_prod_clients() -> None:
    env = _environment(); env["ILAIOS_MICROSOFT_DEVELOPMENT_WEB_CLIENT_ID"] = env["ILAIOS_MICROSOFT_PRODUCTION_WEB_CLIENT_ID"]
...
def test_microsoft_production_redirect_allowlist_rejects_loopback_and_http() -> None:
    for redirect in ("http://app.ilaios.com/auth/microsoft/callback", "https://127.0.0.1/auth/microsoft/callback"):
        env = _environment(); env["ILAIOS_MICROSOFT_PRODUCTION_WEB_REDIRECTS"] = redirect
        with pytest.raises(MicrosoftOIDCConfigurationError): MicrosoftOIDCEnvironment.from_environment(env)
...
def test_microsoft_accepts_personal_and_organizational_concrete_issuers() -> None:
    personal = verified_microsoft_identity(_claims(issuer=CONSUMER_ISSUER)); organization = verified_microsoft_identity(_claims(issuer=ORG_ISSUER))
...
def test_microsoft_identity_key_is_namespaced_by_verified_issuer() -> None:
    personal = verified_microsoft_identity(_claims(issuer=CONSUMER_ISSUER, subject="same-sub")); organization = verified_microsoft_identity(_claims(issuer=ORG_ISSUER, subject="same-sub"))
...
def test_microsoft_rejects_generic_or_attacker_issuer() -> None:
    for issuer in ("https://login.microsoftonline.com/common/v2.0", "https://login.microsoftonline.com/organizations/v2.0", "https://attacker.example/tenant/v2.0"):
        with pytest.raises(CentralIdentityError): verified_microsoft_identity(_claims(issuer=issuer))


def test_microsoft_production_redirect_must_be_exactly_allowlisted() -> None:
    config = MicrosoftOIDCEnvironment.from_environment(_environment()); allowed = "https://app.ilaios.com/auth/microsoft/callback"
    assert config.require_production_web_redirect(allowed) == allowed
    with pytest.raises(MicrosoftOIDCConfigurationError, match="not allowlisted"): config.require_production_web_redirect(f"{allowed}/extra")
