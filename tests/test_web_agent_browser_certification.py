    ...
    admissions = [
        {
            ...
            "action": action,
            "risk": "low" if action != "screenshot" else "medium",
            "admission_decision": "admitted",
            "admission_proven": True,
            "reserved_minor": 1,
        }
        for index, action in enumerate(("open", "snapshot", "screenshot", "reload", "close"), 1)
    ]
    return {
        "schema_version": 2,
...
def test_rejects_stale_source_sha(tmp_path: Path) -> None:
    ...
    with pytest.raises(WebAgentBrowserCertificationError, match="source_sha drifted"):
        verify_browser_e2e_evidence(summary_path=path, expected_source_sha="f" * 40)
...
def test_rejects_missing_governed_admission(tmp_path: Path) -> None:
    ...
    with pytest.raises(WebAgentBrowserCertificationError, match="canonical_admission_proven drifted"):
...
def test_rejects_state_changing_actions(tmp_path: Path) -> None:
    ...
    path = _write_summary(tmp_path, document)
    with pytest.raises(WebAgentBrowserCertificationError, match="state_changing_browser_actions drifted"):
        verify_browser_e2e_evidence(summary_path=path, expected_source_sha=_SOURCE_SHA)
