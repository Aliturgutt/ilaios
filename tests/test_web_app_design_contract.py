...
def test_unmeasured_reference_stays_explicitly_not_reference_exact() -> None:
    ...
    assert (
        contract.design_system.reference_fidelity_status
        == "BASELINE_ONLY_NOT_REFERENCE_EXACT"
    )
    assert any(
        "unproven" in item for item in contract.acceptance_requirements
    )
