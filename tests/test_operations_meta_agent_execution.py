...
def test_provider_failure_classification_exposes_only_bounded_metadata() -> None:
    sensitive_marker = "sensitive-token-never-log-this"
    cause = ValueError(sensitive_marker)
    failure = AIProviderTransportError(
        f"provider response included {sensitive_marker}",
        retryable=True,
    ...
    assert sensitive_marker not in classification
