...
def test_contract_is_deterministic_and_has_zero_runtime_authority() -> None:
    ...
    assert first.interaction_chain == (
        "ui-event", "api-mutation-contract", "authorization", "policy", "admission",
        "approval-when-required", "domain-transition", "validation", "audit",
        "evidence", "notification-realtime-projection",
    )
...
def test_required_workflow_approval_evidence_state_machines_are_exact() -> None:
    ...
    assert tuple(state.state_id for state in machines["Workflow"].states) == ("Planning", "Executing", "Validation", "Delivery", "Completed")
    assert tuple(state.state_id for state in machines["Approval"].states) == ("Pending", "Reviewing", "Approved", "Rejected")
    assert tuple(state.state_id for state in machines["Evidence"].states) == ("Generated", "Reviewed", "Verified", "Failed")
...
def test_non_realtime_spec_uses_notification_projection_only() -> None:
    ...
    workflow = next(machine for machine in contract.machines if machine.machine_id == "Workflow")
...
def test_missing_phase3_action_binding_fails_closed() -> None:
    spec = _spec()
    auth = compile_web_app_auth_contract(spec, project_id="project-phase4")
    actions = tuple(item for item in auth.actions if item.action_id != "action:project.manage")
