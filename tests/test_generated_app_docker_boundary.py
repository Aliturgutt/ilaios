...
class _RecordingBoundary(DockerSecureCommandBoundary):
    ...
    def _docker_run(self, args: tuple[str, ...], *, timeout_seconds: int) -> subprocess.CompletedProcess[str]:
        assert timeout_seconds == 45
...
def test_boundary_builds_fail_closed_docker_isolation_command(tmp_path: Path) -> None:
    ...
    result = boundary.execute(workspace, RuntimeCommand("build", ("pnpm", "run", "build"), "app"), _policy())
...
def test_boundary_rejects_working_directory_escape(tmp_path: Path) -> None:
    ...
    with pytest.raises(SoftwareFactoryError, match="escapes workspace"):
        _RecordingBoundary().execute(workspace, RuntimeCommand("build", ("pnpm", "run", "build"), "../outside"), _policy())
...
def test_boundary_keeps_failed_command_as_failed_evidence(tmp_path: Path) -> None:
    ...
    result = boundary.execute(workspace, RuntimeCommand("test", ("pnpm", "test"), "."), _policy())
    assert result.passed is False
    assert result.exit_code == 9


def test_boundary_kills_host_output_flood_before_control_plane_memory_growth(tmp_path: Path) -> None:
    executable = tmp_path / "fake-docker"
    executable.write_text("#!/bin/sh\ni=0\nwhile [ \"$i\" -lt 20000 ]; do\n  printf x\n  i=$((i + 1))\ndone\n", encoding="utf-8")
    executable.chmod(0o755)
    boundary = DockerSecureCommandBoundary(runtime_image="ilaios/generated-runtime:test", docker_executable=str(executable), output_limit_bytes=16_384)
