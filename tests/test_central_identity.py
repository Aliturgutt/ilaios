...
def test_authenticated_account_can_link_microsoft_github_and_email() -> None:
    ...
    service.link_identity(authenticated_user_id=account.user_id, authenticated_tenant_id=account.tenant_id, identity=_identity(IdentityProvider.MICROSOFT, "ms-sub-1", issuer=MICROSOFT_TEST_ISSUER))
    service.link_identity(authenticated_user_id=account.user_id, authenticated_tenant_id=account.tenant_id, identity=_identity(IdentityProvider.GITHUB, "gh-user-1"))
    service.link_identity(authenticated_user_id=account.user_id, authenticated_tenant_id=account.tenant_id, identity=_identity(IdentityProvider.EMAIL, "User@Example.com", email="user@example.com", verified=True))
...
def test_identity_linked_to_another_account_is_rejected() -> None:
    ...
    service.link_identity(authenticated_user_id=first.user_id, authenticated_tenant_id=first.tenant_id, identity=_identity(IdentityProvider.GITHUB, "github-sub-1"))
    with pytest.raises(CentralIdentityError, match="already linked to another account"):
        service.link_identity(authenticated_user_id=second.user_id, authenticated_tenant_id=second.tenant_id, identity=_identity(IdentityProvider.GITHUB, "github-sub-1"))
...
def test_cross_tenant_link_attempt_is_rejected() -> None:
    ...
    with pytest.raises(CentralIdentityError, match="authenticated tenant mismatch"):
        service.link_identity(authenticated_user_id=account.user_id, authenticated_tenant_id="tnt_other", identity=_identity(IdentityProvider.MICROSOFT, "ms-sub-1", issuer=MICROSOFT_TEST_ISSUER))
...
def test_account_linking_is_idempotent_for_same_user() -> None:
    ...
    first = service.link_identity(authenticated_user_id=account.user_id, authenticated_tenant_id=account.tenant_id, identity=external)
    second = service.link_identity(authenticated_user_id=account.user_id, authenticated_tenant_id=account.tenant_id, identity=external)
    assert second == first
