...
class _Transport(OpenRouterTransport):
    ...
    def post_json(self, url: str, *, headers: Mapping[str, str], body: Mapping[str, object], timeout_seconds: float) -> OpenRouterJsonResponse:
        raise AssertionError("submission is outside polling test scope")

    def get_json(self, url: str, *, headers: Mapping[str, str], timeout_seconds: float) -> OpenRouterJsonResponse:
        self.get_calls.append((url, headers, timeout_seconds))
        return self.response

    def get_bytes(self, url: str, *, headers: Mapping[str, str], timeout_seconds: float) -> OpenRouterByteResponse:
...
def test_openrouter_in_progress_status_is_normalized_to_running() -> None:
    poller = OpenRouterManagedVideoGenerationJobPoller("server-secret", transport=_Transport(OpenRouterJsonResponse(200, {"id": "job-001", "status": "in_progress"})))
...
def test_completed_observation_binds_content_url_and_provider_usage_cost() -> None:
    poller = OpenRouterManagedVideoGenerationJobPoller("server-secret", transport=_Transport(OpenRouterJsonResponse(200, {"id": "job-001", "status": "completed", "generation_id": "generation-001", "usage": {"cost": 0.484}})))
    observation = poller.poll("job-001")
    assert observation.status is ProviderJobStatus.SUCCEEDED
    assert observation.output_asset_ids == ("https://openrouter.ai/api/v1/videos/job-001/content",)
...
def test_terminal_poll_recovers_cost_from_generation_metadata() -> None:
    transport = _SequenceTransport((OpenRouterJsonResponse(200, {"id": "job-001", "status": "completed", "generation_id": "generation/001"}), OpenRouterJsonResponse(200, {"data": {"id": "generation/001", "total_cost": "0.484001"}})))
    observation = OpenRouterManagedVideoGenerationJobPoller("server-secret", transport=transport).poll("job-001")
    assert observation.status is ProviderJobStatus.SUCCEEDED
    ...
    assert [call[0] for call in transport.get_calls] == ["https://openrouter.ai/api/v1/videos/job-001", "https://openrouter.ai/api/v1/generation?id=generation%2F001"]


def test_generation_metadata_without_numeric_total_cost_stays_fail_closed() -> None:
    transport = _SequenceTransport((OpenRouterJsonResponse(200, {"id": "job-001", "status": "completed", "generation_id": "generation-001"}), OpenRouterJsonResponse(200, {"data": {"id": "generation-001"}})))
    observation = OpenRouterManagedVideoGenerationJobPoller("server-secret", transport=transport).poll("job-001")
...
def test_generation_metadata_http_failure_stays_fail_closed() -> None:
    transport = _SequenceTransport((OpenRouterJsonResponse(200, {"id": "job-001", "status": "completed", "generation_id": "generation-001"}), OpenRouterJsonResponse(404, {"error": "not found"})))
    observation = OpenRouterManagedVideoGenerationJobPoller("server-secret", transport=transport).poll("job-001")
    with pytest.raises(OpenRouterManagedVideoRuntimeError, match="usage cost evidence"):
        actual_cost_microusd_from_observation(observation)
...
def test_cost_settlement_requires_terminal_provider_evidence() -> None:
    observation = OpenRouterManagedVideoGenerationJobPoller("server-secret", transport=_Transport(OpenRouterJsonResponse(200, {"id": "job-001", "status": "pending"}))).poll("job-001")
...
def test_terminal_observation_without_usage_cost_keeps_reservation_unsettled() -> None:
    observation = OpenRouterManagedVideoGenerationJobPoller("server-secret", transport=_Transport(OpenRouterJsonResponse(200, {"id": "job-001", "status": "completed"}))).poll("job-001")
    with pytest.raises(OpenRouterManagedVideoRuntimeError, match="usage cost evidence"):
        actual_cost_microusd_from_observation(observation)


def test_failed_job_preserves_usage_cost_when_provider_reports_it() -> None:
    observation = OpenRouterManagedVideoGenerationJobPoller("server-secret", transport=_Transport(OpenRouterJsonResponse(200, {"id": "job-001", "status": "failed", "error": {"code": "provider_error", "message": "failed"}, "usage": {"cost": "0.050001"}}))).poll("job-001")
