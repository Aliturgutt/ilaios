"""Regression contract for exact-master Web App Factory status publication."""
...
WORKFLOW_PATH = (
    ...
    / ".github"
    / "workflows"
    / "web-app-factory-exact-master-status.yml"
)
CONTINUATION_PATH = (
    Path(__file__).resolve().parents[1]
    / ".github"
    / "workflows"
    / "web-app-factory-continuation.yml"
)
...
def test_status_is_derived_only_from_master_push_completion() -> None:
    text = _workflow_text()
    assert "- Web App Factory Continuation Gate" in text
...
def test_status_fails_closed_and_validates_sha() -> None:
    ...
    assert "'context': 'ilaios/web-app-factory-exact-master'" in text
    assert "'target_url': os.environ['TARGET_URL']" in text
    assert "assert response.status == 201" in text


def test_continuation_gate_tracks_status_publisher_changes() -> None:
    text = CONTINUATION_PATH.read_text(encoding="utf-8")
    assert "'.github/workflows/web-app-factory-exact-master-status.yml'" in text
