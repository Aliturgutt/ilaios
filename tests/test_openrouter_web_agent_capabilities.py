...
def test_free_openrouter_bootstrap_covers_all_five_web_proposal_capabilities(
    ...
    assert WEB_GOVERNED_AI_CAPABILITIES.issubset(advertised)
...
def test_free_openrouter_never_routes_browserqa_or_independent_verifier(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "services.openrouter_agent_catalog.urllib.request.urlopen",
    ...
    assert configuration is not None
    advertised = configuration.provider_capabilities["openrouter"]
    assert "web.verify" not in advertised
    assert "evidence.verify" not in advertised
    with pytest.raises(GovernanceError):
        configuration.adapter.select("web.verify")
    with pytest.raises(GovernanceError):
        configuration.adapter.select("evidence.verify")
