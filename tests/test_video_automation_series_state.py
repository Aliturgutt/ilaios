...
from services.runtime.scheduler import SchedulingError, WorkerProfile, WorkerScheduler
...
def test_series_scheduler_uses_existing_scheduler_and_never_invokes_provider(
    ...
    orchestrator = SeriesEpisodeScheduler(state_store=store, scheduler=scheduler)
...
def test_scheduler_failure_marks_same_episode_incomplete_for_resume(tmp_path: Path) -> None:
    ...
    with pytest.raises(SchedulingError, match="no worker"):
