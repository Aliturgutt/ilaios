...
def _handler(*, actual_id: str | None = None, status: str = "BLOCKED"):
    ...
    handler.server = SimpleNamespace(coordinator=coordinator, bearer_token="test")
    handler._authenticated_session = Mock(return_value=SimpleNamespace(principal_id="owner", tenant_id="tenant"))
    handler._start_execution = Mock()
    handler._send_json = Mock()
    return handler, coordinator


@pytest.mark.parametrize("factory", FACTORIES, ids=lambda x: x.capability_id)
def test_each_canonical_factory_returns_actual_route_and_does_not_start_blocked_work(factory):
    ...
    handler._start_execution.assert_not_called()
    coordinator.prepare.assert_called_once()


@pytest.mark.parametrize("factory", FACTORIES, ids=lambda x: x.capability_id)
def test_each_factory_rejects_conflicting_objective_before_prepare(factory):
...
@pytest.mark.parametrize("factory", FACTORIES, ids=lambda x: x.capability_id)
def test_each_factory_rejects_wrong_actual_route_before_start(factory):
    other = next(x for x in FACTORIES if x.capability_id != factory.capability_id)
...
@pytest.mark.parametrize("factory", FACTORIES, ids=lambda x: x.capability_id)
def test_each_factory_rejects_unauthenticated_session_before_prepare(factory):
    handler, coordinator = _handler(actual_id=factory.capability_id)
    handler._authenticated_session.side_effect = PermissionError("session denied")
...
@pytest.mark.parametrize("factory", FACTORIES, ids=lambda x: x.capability_id)
def test_each_factory_adapter_maturity_is_reported_not_assumed(factory):
...
def test_rejects_noncanonical_factory_before_prepare():
...
@pytest.mark.parametrize("factory", FACTORIES, ids=lambda x: x.capability_id)
def test_each_factory_rejects_multi_factory_objective_before_prepare(factory):
    other = next(x for x in FACTORIES if x.capability_id != factory.capability_id)
    handler, coordinator = _handler(actual_id=factory.capability_id)
    with pytest.raises(ValueError, match="conflicts"):
...
@pytest.mark.parametrize("factory", FACTORIES, ids=lambda x: x.capability_id)
def test_each_factory_returns_blocker_from_coordinator(factory):
    descriptor = _ADAPTER_DESCRIPTORS[factory.capability_id]
    if descriptor.maturity.value == "VERIFIED_FINISHED_PRODUCT_ADAPTER":
