...
def test_shutdown_is_noop_after_serve_loop_has_returned(monkeypatch) -> None:
    server = _bare_server()
    calls: list[str] = []
    ...
    monkeypatch.setattr(
        ...
        "shutdown",
        lambda _self: calls.append("shutdown"),
    )
...
def test_serve_loop_marks_only_the_live_shutdown_window(monkeypatch) -> None:
    server = _bare_server()
    active_during_parent: list[bool] = []

    def fake_serve_forever(_self, *, poll_interval: float = 0.5) -> None:
        del poll_interval
